...
('STEP-5-PROMPT.txt', 'tool', "Step 5's prompt, as plain text to paste",
  'GENERATED by tools/build-board.py from build-sequence.md, alongside BUILD-BOARD.html and from '
  'the same parse — so the three copies of this prompt cannot drift. It exists because step 5 is '
  'the one prompt the owner pastes by hand and a .txt is the easiest thing to open and select all. '
  'DO NOT EDIT IT: edit the fenced block in build-sequence.md step 5 and regenerate. It was '
