...
DOCS = [
 ...
 ('tools/probe/run-verify-account-deletion.py', 'tool', 'Account-deletion harness',
  ...
  'Admin-API user count before and after so a leak is loud. It does not assert that the email '
  'arrived: the app logs Resend\'s status and id and the owner reads the inbox. DW-22 CLOSED '
  '2026-09-11 (Story 3.9) — a reading key now exists and `last_event` is readable, so this is a gap '
  'in the harness rather than in the credentials, and Epic 12 is where a harness first asserts it. '
  'Plus '
 ...
 ('tools/appendix-a-prompts.py', 'tool', 'Appendix A prompts — generator',
  'Generates APPENDIX-A-PROMPTS.html by extracting the fenced prompt under every ### A heading of '
  'EXPERIENCE.md Appendix A — retyped nowhere, so the page cannot drift from the spine — grouped as '
  'the two sessions the preamble names: A7 with A8, then the rest in heading order. The export '
  'warning and the Handlebars line are lifted from the preamble the same way. --check fails when '
  'the page is stale against the spine, and the gate runs it.'),
...
('STEP-6-PROMPT.txt', 'tool', "The current step's prompt, as plain text to paste",
  'The step-6 prompt (the story breakdown) as a plain-text file, GENERATED by tools/build-board.py from the same parse '
  'that feeds the board, so the two copies cannot drift; --check fails if either is stale. '
 ...
 ('planning-artifacts/STORY-BOARD.html', 'live', 'Story board — the tracker for the build',
  "Ruling R-84: where the owner watches the whole build. Every epic and story, colour-coded by status "
  "and laid out as lanes that fit one screen; each phase's prompt copyable from the card; a story opens "
  "to its plain-English scope, its acceptance criteria, its commits, its questions for the owner and — "
 ...
 ('design/claude-design-export/Inflozo/S11e Manage Keys Popup.dc.html', 'record', 'Manage keys — the popup frame (S11e)',
  ...
  'as this: a 900×743 window holding the SAME content as the long screen with nothing removed, the '
  'three credential rows on the left and everything the customer reads rather than types in a context '
  'rail on the right, Test connection at the rail’s foot. It supersedes S11d’s chrome and B20’s stacked '
  'rows FOR THIS SURFACE ONLY. IT LIVES IN THE EXPORT BESIDE THE OTHER FRAMES BY THE OWNER’S RULING '
  '(Story 3.6, Question 4, option 1, 2026-09-10): one design authority, so tokens.test.ts and the export’s '
  'own checks cover it. It arrived in its own folder with a support.js byte-identical to the export’s, '
  'so only the frame moved; the rendered .png he looked at is screenshots/s11e-manage-keys-popup-owner.png. '
  'A record: dated, never edited (R-74).'),
]
...
def check():
    ...
    for tool, art, src in (('build-board.py', 'BUILD-BOARD.html', 'build-sequence.md'),
                           ...
                           ('appendix-a-prompts.py', 'APPENDIX-A-PROMPTS.html',
                            'EXPERIENCE.md Appendix A'),
