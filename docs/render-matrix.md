...
Each case is rendered exactly as the editor draws it: `renderCanvas` with the input `/pilots`' `paint()` builds, written
into the canvas document `/pilots/frame` serves (`pilotsCanvasDocument()`), with `data-mode` set, `js-enabled` on every
