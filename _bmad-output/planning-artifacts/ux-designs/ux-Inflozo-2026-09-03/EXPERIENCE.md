...
| **Template Switcher** | S4a · `D5 Canvas Markers and Template Switcher.dc.html` D5b, complete | top bar | Home · Post · Page · Tag · Author · **Membership (Signup / Signin / Member Home)** · 404 · conditional Private · custom templates · + New template (FR-D6). Since **R-171** (2026-09-21) each row is View as's row — a Tabler glyph, the name over one line saying what the template is ("Custom template" for a custom one), the state mark trailing with its word heard rather than printed — and the list scrolls inside its card. Since **R-172** the row in force, in this list and View as's, is highlighted with `coral-tint` rather than ticked |
| **Layers** | B7 · D8e focused row *(as R-126 amends B7, 2026-09-18)* | `L` · left panel | Ordered sections; a **Site-wide** group above the page's own, both drawn the same with a hairline between, the row's only control a `⋯`. **Choosing a row brings its section into view** — the canvas scrolls smoothly and the section comes to rest a little below the top edge, never against it (**R-156**, owner, 2026-09-20). Only from Layers: a press ON the canvas is already looking at the section. A **sticky** section is exempt, because it travels with the viewport and is in view wherever the page is — executed: a stuck root reports `rect.top` 0 and `offsetTop` 2425, so neither says where it lives |
...
| **Ghost's Own Surfaces** | S4a *(which draws neither — each is drawn to Ghost's own look, recorded in `MEASUREMENTS.md` §55)* | every page canvas of a project linked to a connected site | What Ghost adds to every page by itself, drawn where Ghost puts it (FR-H5, Story 5.21): its **announcement bar** as a strip across the very top of the page, above the header, pushing the page down — the site's own words in its own colour, only for the visitors it reaches (View as) — and Portal's floating **Subscribe** button in the bottom-right corner where the site owner switched it on, never below 640px. Neither can be pointed at, pressed, selected, moved or saved; neither is on a template surface; the editor re-reads them from Ghost each time it opens |
...
| **"Gated content — shown with sample text"** | the body on a members-only canvas — where the gated part begins, for a visitor who may read it; a visitor who may not meets the cut and the paywall (R-199) |
...
| **Dynamic placeholder** | A small **`{}`** button beside a text field's LABEL, and **nothing under the field** — its menu lists the placeholders *that field* offers here, each its code over one line saying what it does, with **Copy** and **Insert** on the right as Tabler glyphs that name themselves on hover — Copy answering with a tick for two seconds, Insert closing the menu, and the description never cropped (R-188). A field with nothing to offer carries no button at all (absent, never greyed). `{page_number}` is offered only while the canvas shows page 2 and only in a section that is not site-wide, and prints nothing on page 1, a post, a standalone page or the 404 — but typed by hand anywhere it is kept and substituted by the same one rule. **This is the one way every placeholder is reached, existing and future** — see `DESIGN.md` § Components, *The placeholder menu*, which is normative (R-182, R-185, R-186, R-187, R-188; Story 5.16a) |
...
| **Site-wide singleton** | Header, announcement bar and footer are one shared instance shown in every template's Layers as the Site-wide group with its template count (B7's pinned card and globe went with R-126, 2026-09-18). Cannot be duplicated. Deleting or hiding one confirms that it affects every template (FR-D5). **The Section Picker offers them on every canvas** and they land in the Site-wide group rather than at the invoked position, because the stack order is derived, not stored; **a second one in the same category replaces the first**, as one edit, so `⌘Z` restores it. **A site-wide design is marked on its picker card by the Kit's `Globe`, with a hover `title` and the same words as its accessible name — never a sentence, a toast or a banner** (R-152, owner, 2026-09-19). Layers still carries no glyph: R-126 stands |
...
| **Paywall Editor** | **members disabled on the site** (`members_signup_access` = none): `C3b`'s shape — what Ghost does instead, two numbered steps and a Re-check. **What Ghost does is unlock nothing** (R-198, 2026-09-26): a post for members still stops at the cut, and its box can sign nobody up (`post-gating.js`, read in source on both majors; **recorded by Story 5.20 on T3 with Subscription access set to Nobody, `MEASUREMENTS.md` §54**) — so C3b's sentence says that, not "nothing on the site is gated". **The recording corrected one clause:** Ghost does not always stop loading Portal — it keeps it wherever donations stay on, which a Stripe-connected site does by default — and Portal then answers "Memberships unavailable, contact the owner for access." So the sentence ends "…but nobody can sign up there." and says nothing about Portal (`apps/web/lib/paywall.ts`). **Not** "no paid tiers": Ghost's paywall renders on every gated post — members-only, paid or tier-gated — whether or not a paid tier exists (`content-cta.hbs` carries its own `visibility="members"` branch; VERIFY-AT-BUILD item 11), so a free newsletter gating posts to members is exactly a site this editor serves. `C3b` has said the same since 2026-09-04 — "Members are switched off … subscription access is set to Nobody" (A7 item 12) | — | — | the same shape, naming the setting |
| **Suggestions** | S13c | — | — | — |
...
| **S4d** Member State Preview *(corrected 2026-09-04, A7 item 18)* | now "Logged out user · Free member · Paid member", "2 not viewed" beside the toggle and "Gated content — shown with sample text" on the gated body; until 2026-09-04 tier-level previews — Orbit Supporter $5, Patron $12, Founding $120 | FR-D16: **three states** — Logged out user / Free member / Paid member. `comped` previews as Paid, differing in billing rather than access. **B9 already says "only" and agrees with the PRD**; S4d is the outlier and moves. Add the two things neither frame has: the **unviewed-states marker** beside the toggle, and the **"Gated content — shown with sample text"** indicator on a gated body. **Built at Story 5.14 with the owner's two rulings (2026-09-21):** the marker is a coral dot on each unviewed row of the menu and nothing beside the toggle (**R-169**), and each visitor has one name — S4d's row title — on the button, in the menu, in the panel and in the announcement, so "Anonymous" is not used (**R-170**) |
...
| **S5** Section Picker | a **`Free only`** toggle in the rail footer (`:73-76`), and a two-cell **sun/moon segmented** in the header (`:82-85`, S5c `:236-239`) | **Both re-specified, Story 5.10's Create, 2026-09-19.** *Ruling R-150:* the toggle is **dropped** — R-77's reasoning generalised from Site Remix to browsing the library: the canvas is open, the ✦ Pro tag is the only Pro signal, and FR-L3's exit sheet is the enforcement. *Ruling R-151:* the segmented becomes **R-132's one button that swaps its glyph**, in the segmented's drawn position, flipping the same `mode` the canvas holds — so the picker and the page behind it can never disagree. Everything else in S5a and S5c stands, S5c's *"app chrome stays light"* included. **And a third, from the owner's test of the deployed story — *ruling R-153*, 2026-09-20:** `:88`'s **CSS multi-column** (`column-width:300px`) draws every card at a column's width and its own content's height, so a header came out a ~20px sliver and the hover wash — which **is** the `Add` button, `top:0 → bottom:41px` — **cropped the pill on any short section**. The grid is now **four columns of a fixed row unit, packed dense**; a card's **span is measured from the design's own drawn aspect** (a band spans two columns, a feed two rows — `apps/web/lib/picker.ts`'s `spanFor`); and the `Add` is an **icon in the card's footer strip**, centred between the name and the tier badge, on hover, on focus, and always where there is no pointer. The wash stays as decoration. **S5a and S5c are superseded on the grid's shape and the `Add`'s seat alone**. **And on four more points, from his second pass the same day — *ruling R-154*:** the rail leads with an **`All sections`** row carrying its own derived count (S5a gives no way back to everything once a category is chosen), the heading beneath it is **`CATEGORIES`**, the meta line is the **count alone** (`:81`'s *· shown in your pack: {pack}* states the surface's own premise and never varied), and a hovered card takes a **1px `--color-coral` border and nothing else** — `:89-100`'s wash hides the miniature the card exists to show, and the 2px lift moves the card under the pointer. **The span rule is the standard every future design inherits**: a design's shape is measured, never authored |
| **B1** Design navigation | **B1b** draws the counter and arrows in an **ink pill at the section's top-LEFT**, carrying **Shuffle** and a `⋯` (`:425-440`) — while `S4 Editor.dc.html` **S4b**`:181` and `S6 Variant Shuffle.dc.html``:67` both draw the same counter and arrows in the **white quick-action pill at `top:10px;right:10px`**, with no Shuffle and no `⋯` | **Reconciled at Story 5.11's Create, 2026-09-20.** S4b and S6 agree with each other, the pill they draw is **built** (`section-pill.tsx`, Story 5.4) and its corner is already ruled (**R-125**), and **R-118** said in the owner's own terms that `◀ ▶` arrive with 5.11 **in S4b's pill** — so **S4b + S6 govern the pill** (order: `◀` · counter · `▶`, then the divider, then Duplicate · Delete · grip) and **B1b governs the affordance**, which is its real claim: *"the same counter and arrows riding on the section, so a user never has to look right to change a design"*. B1b's ink colour, its top-left seat and its `⋯` are **not built** (R-126 keeps the `⋯` as the Layers row's one control). *Ruling **R-159** (owner, 2026-09-20, Story 5.11's Q2, option 2):* **Shuffle is built in BOTH places** — S6`:140`'s **`Try a design`** card at the foot of the panel's Design block, and a Shuffle control in the section's pill as B1b draws it, seated **with the ring, before the divider**, because the divider in the built pill separates *which design* from *this section*. In the pill it is **icon-only** — the Kit's `Refresh` with its words as name and hover title, through `DESIGN.md:534-536`'s carve-out that R-132 and R-136 already use — because every other control in that pill is a 26px round icon target and a word would be the only text in it; the **words** live on the panel card. Both are **absent** where the ring holds one design (UX-DR3). ***R-159 AMENDED by the owner's test of the deployed page, 2026-09-20 (Story 5.11's findings 2–4):*** Shuffle keeps **one seat, the pill's** — the `Try a design` card is not drawn, nor is B1a's `Cycle designs` footer, and the counter prints `7 of 18` beside its `Design` label rather than repeating the word. In the editor Shuffle is therefore pointer-only; `[` and `]` reach every design it could |
...
|---|---|---|
| **The reader** (opened it second) | **Editor**, read-only | B5a: a bar — **"You are editing this site somewhere else — you are reading along here"** (R-189) + **Request editing**, which swaps to **Asking…** while in flight. The canvas stays fully legible; the sidebar dims to 55%, and **every control that would edit is disabled** — the panel's fields, switches, pickers and buttons, Layers' ⋯ menus, drag and **+ Add section**, the section pill's actions, **Site Remix**, **Undo**, **Redo** and the editing shortcuts (**R-192**) — while what only changes the *view* still works: picking a section, opening a group or a list item to read what is set, Template, View as, dark mode, the device sizes and Preview. On **gaining** the lock it hydrates from the cloud (a reload), so it never edits the page it loaded as a reader (the owner's finding, 2026-09-24; §AD2's nudge flow) |
