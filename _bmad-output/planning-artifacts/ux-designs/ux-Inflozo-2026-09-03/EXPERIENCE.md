...
| **B11** Device Preview *(corrected 2026-09-04, A7 item 6)* | now "There is one control here: device" and the chip "VIEWPORT 390 × 844 · SHOWN AT 55%"; until 2026-09-04 a user "Fit / 55%" zoom control. **B11a also carries the chip "VIEWPORT 1440 × 900 · SHOWN AT 46%", which contradicts S4a`:63`'s resting card filling the height — R-137 (owner, 2026-09-19) rules for B11a on the card's GEOMETRY and for S4a on everything else about it** | FR-D14: **no zoom in v1**; AD-21: fit-to-screen only, never user-driven. Keep the mono chip that states true size and scale; delete the control |
