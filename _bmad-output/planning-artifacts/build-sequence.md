...
>
> **Three things a reader should know about how it was built.** It builds the **re-specification**,
> not the frame, for the six frames drawn on the wrong mechanism, and each of those pages says so in
> a note under its title. Its plan limits are **Appendix F.1's**, not the frames' — four frames
> disagree with F.1 and F.1 governs. And **Appendix A's Claude Design prompts had not been run when
> it was built** (the export carries no D-canvases), so the pages for surfaces with no frame are
> extrapolated exactly as the spine describes and **should be re-checked against the new frames once
> those prompts have run**.
>
...
> The prototype is **disposable by design** the day the dynamic UI matches it.
>
...
fill in on his behalf: the gate is the walk, not the build.
