...
SIX THINGS ALREADY SETTLED THAT THE SKILL OR THE PRD WOULD OTHERWISE MAKE YOU GUESS:
  ...
  - The skill's technical-layer, file-churn, independence and single-session checks will all
    ...
    section 8 and go on; none of them is a stop.
  - Where EXPERIENCE.md's Information Architecture gives a surface only "→ A<n>" (no frame yet),
    name that prompt and write the criterion as "matches the A<n> frame once the export carries
    it". A category story's frames are <ID>-<n> <Name>.dc.html and <ID>-0 Category Proof.dc.html.
