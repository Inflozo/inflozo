...
> **The critical path is now STEP 5B — the static prototype.** Step 5 completed on 2026-09-03:
> `ux-designs/ux-Inflozo-2026-09-03/` holds both spines, `status: final`. **Step 6 stays shut until
> the owner has walked 5b's prototype** (R-75). R-30 … R-38 were propagated on 2026-08-31 and step 4
> owes nothing normative; what is left of it is one Claude Design patch pass over the specs those
> rulings name, which blocks nothing. **Step 5 added seven more Claude Design prompts** — Appendix A
> of `EXPERIENCE.md` — and they block nothing either.
...
**Everything to the left of step 5b is done.** Step 5 ran on 2026-09-03 and its spines are final;
what remains on the critical path is the prototype the owner walks, and then stories. See step 5b.
...
**Step 5 is done (2026-09-03). Step 5b — the static prototype — is the critical path and is blocked
by nothing** — see below.
...
> `EXPERIENCE.md`, both `status: final`. **Step 5b is next**, and step 6 stays shut until the
> owner has walked 5b's prototype (R-75).
...
| `EXPERIENCE.md` | The experience spine, `status: final`. Foundation · Information Architecture · Voice and Tone · Component Patterns · State Patterns · Interaction Primitives · Accessibility Floor · Responsive & Platform · **the four journeys and the eight flows** · what the export draws and where it draws the wrong thing · **Appendix A: seven Claude Design prompts** |
...
**Seven Claude Design prompts, in `EXPERIENCE.md` Appendix A.** Six draw surfaces that do not exist
(A1 the first-deploy gates · A2 deploy history with pinning · A3 the drift report · A4 the dashboard
sheets and blocks · A5 the canvas markers and the template switcher · A6 Theme Settings completed);
**A7 corrects six frames whose mechanism changed** — it redraws nothing, it changes what six frames
say. Each is self-contained, names the frame it inherits from, and carries the zero-width-entity
warning.
...
TWO THINGS ABOUT EXPERIENCE.md'S SURFACE LIST, BOTH DECIDED SO YOU DO NOT HAVE TO GUESS:
  ...
  - Some surfaces have no frame yet: EXPERIENCE.md names the frame each is EXTRAPOLATED FROM
    and Appendix A carries a Claude Design prompt for it. Build those from the named frame's
    components, exactly as the spine says. If the owner has run the prompts by the time you
    start, prefer the new frames — check the export before you assume either way.
  - Six frames are DRAWN ON THE WRONG MECHANISM and EXPERIENCE.md re-specifies them (its
    "Drawn, but on the wrong mechanism" table). BUILD THE RE-SPECIFICATION, not the frame:
    keep the frame's components and change what they do. That table is the list.
...
| **① Now — seven Claude Design prompts** | `EXPERIENCE.md` Appendix A. Six draw surfaces that were never drawn (the first-deploy gates · history with pinning · the drift report · the dashboard sheets · the canvas markers · Theme Settings completed); **A7 corrects six frames whose mechanism changed.** They block nothing — run them before or after step 5b, whichever you prefer |
| **② Then — walk step 5b's prototype** | It is one double-click. **Step 6 does not open until you have**, and saying you have walked it is yours to say (R-75) |
