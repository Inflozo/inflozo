...
`@page` is offerable on `page` (where it is meaningful and **must** be honoured — FR-J1 gates `page.hbs` on it to silence GS110), tolerable on `post` (always `true`, so gating on it is a no-op), and **not offerable** anywhere else.
...
Consequence for A31: a 500-treatment variant must not be `{{#get}}`-driven. A 404-treatment variant may be.
...
**`{{content}}` self-gates.** When `access` is false it emits Ghost's upgrade CTA instead of the body — overridden wholesale by `partials/content-cta.hbs`, which is exactly the surface A32's paywall variants compile into.
...
|---|---|---|---|---|---|
| 1 | **`@even` / `@odd` parity** — **the one that matters** | `@even` is true when **`@index`** is even, making the **first** item even | `frame.even = index % 2 === 1` — parity of the **1-based `@number`**, making the **first item ODD** | **Code: the first item is odd** | **Inverts zebra striping across the entire 487-variant library.** Every alternating-row, alternating-column and checkerboard design is backwards, in the same direction, everywhere. Verify this first. |
| 2 | Helpers in error templates | Error templates "shouldn't use any theme helpers, with the exception of `{{asset}}`"; only `error-404.hbs` may | Casper's shipping `error.hbs` uses `{{meta_title}}`, `{{img_url}}`, `{{t}}`, `{{#if}}`, `{{#foreach}}`, `@site.*`; `error-404.hbs` extends `default.hbs` and runs `{{#get}}` | **Code** — the docs' rule is a robustness recommendation (§3c) | A31's error variants lose their designs and fall back to bare markup |
