...
|---|---|---|
| **Gallery row ratios** | A33's gallery card | **Ghost's own script, not a module** *(architect's ruling, 2026-09-03)*. Ghost computes the row ratios for its gallery card and ships the code with the card; Inflozo neither writes it, bundles it nor budgets for it. The registry has no entry because there is nothing of ours to register — the same treatment `cards.js` gets. **A33 styles the result and declares no module.** Stated so the proposal is not made again |
...
| `tabs` | All panels render stacked and visible, **each preceded by its tab label as a heading ONLY WHERE THE PANEL DOES NOT ALREADY CARRY ONE** *(architect's ruling, 2026-09-03)*. Three designs draw their own heading inside the panel — A5-12, A13-11 and A15-10/15 — and would otherwise announce the same label twice. The module's heading exists to make the stacked state navigable; where the panel already has one, that job is done. **The design's own heading is never dropped instead**, because that would change the WITH-JavaScript state too, and the no-JS branch must differ from the JS branch as little as possible. | **no** — hidden panels are unreachable on the canvas |
| `carousel` | The slide track is a native horizontally-scrollable `scroll-snap` strip — **fully usable**, only dots and arrow buttons are hidden. **A module hides only the affordances IT ADDS, never a static element the design drew** *(architect's ruling, 2026-09-03)*: A5-14 Scroller refuses dots and offers a 3 px static rule as its indicator, and that rule is CSS the stylesheet emits, so **it survives with JavaScript off**. The general form matters more than the case — the no-JS state is the server-rendered state minus what the script would have created, and nothing else. | **no** *(owner-ruled — settles A14 vs A15)* — off-screen slides cannot be clicked |
