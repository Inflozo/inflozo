...
- **Signaling** — *amended by **R-191** (owner, 2026-09-24): Supabase Realtime is not used in v1.* Same-browser tabs signal each other over BroadcastChannel at zero cost; everything else hears at the next ~15 s heartbeat, the lock row itself carrying the nudge ("release requested"), the release and the takeover. Executing the Realtime layer (`MEASUREMENTS.md` §50) found it needs a browser-side Supabase client the app deliberately has not got, and a `realtime.messages` policy for a private channel. The cost, accepted with the ruling: a holder on another device learns of a nudge up to ~15 s late, so it can have as little as ~15 s of the ~30 s below to answer.
