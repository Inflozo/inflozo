...
create table public.project_templates (
  ...
  user_id       uuid not null references auth.users(id) on delete cascade,
  template_key  text not null,   -- site|home|index|post|page|tag|author|error|private|tag-paged|author-paged|paywall|custom:{filename}
  ...
  -- The paywall is the first TEMPLATE SURFACE (R-197): the box at `partials/content-cta.hbs`, stored under `paywall`
  -- with at most one instance. Mirrors `supabase/migrations/20260926120000_paywall_template_key.sql` (Story 5.20).
...
create table public.project_template_prefs (
  ...
  updated_at    timestamptz not null default now(),
  primary key (project_id, template_key),
  -- The same keys as `project_templates`, page 2's `tag-paged` and `author-paged` included (Story 5.16), and the
  -- paywall's `paywall` (Story 5.20).
  constraint template_key_shape check (
    template_key in ('site','home','index','post','page','tag','author','error','private','tag-paged','author-paged','paywall')
    or template_key ~ '^custom:custom-[a-z0-9]+(-[a-z0-9]+)*\.hbs$')   -- ONE backslash: DW-193
);
