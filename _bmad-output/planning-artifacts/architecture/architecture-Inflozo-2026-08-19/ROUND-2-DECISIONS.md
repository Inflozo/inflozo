...
| **R2-4** | **Take Round 1's open rider.** *(Corrected on implementation: the single-statement form Round 1 wrote is a **syntax error** — `ALTER DEFAULT PRIVILEGES` takes one object type per statement. Shipped as three statements, verified on PostgreSQL 17.)* **And extend AD-26's per-migration checklist from tables to views and functions.** | Every Round 2 database finding shares this root cause, as did Round 1's `suggestions_public` finding. R2-1 moves the platform in the same direction regardless. |
...
| **R2-10** | **Specify the layer-name slug function and its collision rule.** *(Refined on verification: §7.4 ALREADY rules on this for **custom template names** — "the second is refused and the user renames", prevented at creation, never auto-suffixed. The gap is the **layer-name → section-partial** surface, which has no rule. It may legitimately differ, since a section partial is never referenced by a Ghost page, but it still breaks NFR-6(c1) snapshots and FR-J16 drift.)* | `Hero` and `HERO` slug to the same `hero` — three distinct names produced two slugs. The spike names partial files from layer names, so one section's partial overwrites another's. Compounds with R2-12. *(The "not specified anywhere" note in the Round 2 report was wrong and is withdrawn: §7.4 specifies the rule for custom template names, including the reason auto-suffixing is refused. Only the layer-name surface is unruled.)* |
...
| # | Decision | Evidence |
|---|---|---|
| **R2-16** | **Drop Round 1 decision 25 (splitting `project_treatments`).** Keep the one table. Round 1 decision 26's "nothing to build" then stays true. | Decision 26 names `project_treatments.pagination_design_id` as "already in the schema — nothing to build"; decision 25 splits that table. Whichever lands second falsifies the other's cost. The split turns one table with one RLS policy into three, each needing its own policy, its own `rls.sql` row (AD-26) and its own denormalized `user_id` (AD-6) — permanently — to separate three nullable text columns. *(Verified on implementation: no driver exists anywhere in the tree — `project_treatments` is not mentioned in `prd.md` at all, it is a schema-only construct. Decision 30 already grants the identical cross-epic exemption to the shared notifications schema, so this is precedent, not a new carve-out. Recorded as AD-27 corollary (a0).)* |
