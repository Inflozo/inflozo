...
- **Rule:** the compile function runs at **`maxDuration: 300` (the Pro default) and `memory: 2048 MB` (the Pro default)**. The 40-section stress fixture — 70 section renders over 7 templates, 173 files, 9.75 MB uncompressed — measures **~2.1 s wall and ~290 MB peak RSS**, scoring gscan 0/0: render 770 ms, assemble 14 ms, gscan 210 ms **per pass** (two passes — but see AD-34: those are two gscan **majors**, 4.49.7 and 6.4.2, not one gscan run twice; `MEASUREMENTS.md` §13c measures the pair and states why its numbers are not comparable to this fixture's), the **FR-J17 quality gate 700 ms** and zip 222 ms. The first draft of this AD omitted the FR-J17 gate and counted only one gscan pass; both were measured at the reviewer gate and folded in (`MEASUREMENTS.md` §2, §11). That is still ~140× duration and ~7× memory headroom against a reference 4× faster in cores than Vercel Fluid's 1 vCPU. **§7.1's gscan-boundary split is therefore not built.** Re-measure and revisit if a single compile exceeds 30 s or 1 GB in production.
