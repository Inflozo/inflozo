...
- **Rule:** nothing under `packages/section-runtime`, `packages/ghost-shim` or `packages/theme-compiler` may import `next/*`, `@supabase/*`, `node:fs`, `node:net`, `process.env`, `fetch`, `Date.now`, `Math.random`, or a DOM global it did not receive as an argument. The DOM implementation is injected (`window.document` in the browser, a `jsdom` document in the function). Enforced by a CI dependency boundary check, not by review.
...
- **Rule:** the compile function runs at **`maxDuration: 300` (the Pro default) and `memory: 2048 MB` (the Pro default)**. The 40-section stress fixture — 70 section renders over 7 templates, 173 files, 9.75 MB uncompressed — measures **~2.1 s wall and ~290 MB peak RSS**, scoring gscan 0/0: render 770 ms, assemble 14 ms, gscan 210 ms **per spec** (two specs, v5 and v6, so ~420 ms), the **FR-J17 quality gate 700 ms** and zip 222 ms. The first draft of this AD omitted the FR-J17 gate and counted only one gscan pass; both were measured at the reviewer gate and folded in (`MEASUREMENTS.md` §2, §11). That is still ~140× duration and ~7× memory headroom against a reference 4× faster in cores than Vercel Fluid's 1 vCPU. **§7.1's gscan-boundary split is therefore not built.** Re-measure and revisit if a single compile exceeds 30 s or 1 GB in production.
...
| Files & directories | kebab-case everywhere on disk, including the 484 design directories. Emitted theme filenames derive from slugified layer names (§7.4) and are deterministic. |
