...
`tools/stress/test-renderer-agreement.js` renders one source through **both** emitters and compares
...
- **Rule (⚠️ OPEN — Storage has five ways to be deleted and no way to be undone)** *(raised by the reliability round, 2026-08-20; register item 38)*: this AD states that Storage has no PITR and then leaves the consequence unaddressed. Counted from this spine's own text, **five deletion paths across four epics** can remove objects — FR-A5's account purge (E2), FR-B3's project delete and FR-J7's artifact retention (E7), FR-C6's 90-day snapshot orphan rule (E3), FR-K3's over-quota rules (E8) — and **two buckets are called unrecoverable by this document**: `site-snapshots` ("the only irreplaceable bucket", a customer's pre-Inflozo theme) and `deploy-artifacts` ("NOT regenerable", corrected by decision D10 because compile is pure in the *library*, which FR-J14 moves monthly). **The risk is not durability.** Supabase Storage is object storage and bit-rot is not the failure mode. The risk is **deletion** — five chances for a bug, a bad predicate or a mistaken purge to take something no one can recreate, with nothing behind it. Postgres gets PITR for exactly this class of accident and Storage gets nothing. **The decision is the owner's and is not made here**; it is recorded as open rather than resolved by an architect's preference, because it trades a small recurring cost against an unrecoverable failure and that is a business call. What this AD *does* now bind: **no epic may add a sixth deletion path without this question being answered.**
