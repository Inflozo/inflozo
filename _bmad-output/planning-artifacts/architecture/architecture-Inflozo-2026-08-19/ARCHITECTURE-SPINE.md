...
- **Multi-region and read replicas.** One region — **`fra1` since 2026-09-06**, and `iad1` before it. The complaint this bullet waited for arrived: the owner's third test of story 1.5 reported the app slow and read `bom1::iad1::…` off his own response, and the two platforms' APIs confirmed the split — Vercel functions in `iad1` (Washington), the Supabase project `Inflozo` in `eu-central-1` (Frankfurt), so every render and every server action crossed the Atlantic to reach the database. `fra1` **is** eu-central-1, so the function and the database are now in one region; the setting is the Vercel project's `serverlessFunctionRegion` / `resourceConfig.functionDefaultRegions`, applied on the next deployment, and the figures either side of it are in that story's spec. Still one region, and read replicas are still deferred. Deployed sites have zero runtime dependence on Inflozo (P5), so the blast radius of a single region is editing sessions only.
