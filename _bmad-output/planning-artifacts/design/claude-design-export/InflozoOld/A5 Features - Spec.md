...
**Sixth pass — design patch pass five (this document's current state), 3 September 2026.** A small pass, and mostly mechanical. **Image focus stops being a private copy:** the enumeration §0 carried is replaced by a reference to **P0·9**, where the control is now defined once for the whole library and carries **both** axes, so the side-to-side half added by the owner's ruling of 3 September 2026 is in force here without a word of it being restated. **Both registry questions in Two findings came back ruled**, and each is written in where it lands: `tabs` emits its tab-label heading **only where a panel does not already carry one**, which settles 12 Tabs' double announcement without touching that design's own h3; and **a module hides only the affordances it adds, never a static element the design drew**, which keeps **14 Scroller's 3 px rule with JavaScript off**. **Nothing was redesigned, nothing renumbered, no frame moved, no arrangement changed, no control renamed, and no value added or removed.** Two things this pass could not settle are recorded rather than guessed, both marked **OPEN FOR THE OWNER** in the closing Patch notes.
...
**Two findings**, at the end, and **both are now answered** (3 September 2026): the double heading `tabs` gave 12 with JavaScript off, and the missing registry sentence for 14's rule. Each ruling is written into the design it lands on as well as into that section.
...
**Image fields — the shared control, cited rather than restated.** Every `itemImage` carries **Image focus**, reachable from the Image Picker popover — never a hidden field and never a panel row — on every design that draws one: 7 Alternating Media, 8 Media Top, 9 Bento's item 1, 12 Tabs, 13 Spotlight and 14 Scroller. **The control and its values are P0·9's**, defined there once for the whole library and deliberately not restated here: both axes, their defaults, the one-control-per-image-slot rule and the rule for narrowing an axis all live in that section. Until this pass this document wrote out its own copy of the up-and-down values, which is the failure the owner ruled on as *one control name means one set of values*; the enumeration is gone and the reference stands in its place, so **the side-to-side axis added by the owner's ruling of 3 September 2026 is in force here by reference rather than by restatement** — a wide photograph cropped into one of these frames keeps the side the site chooses. **A5 narrows neither axis and constrains nothing:** every picture in the category is cropped into a frame of the design's own shape — 3:2 on 7, 12 and 13, 16:9 on 8 and 14, and on 9 the large tile's own box, whose height the words beside it set — so both axes always have work to do. **One control per image slot, never one for the section:** the five designs that draw a picture per item draw one focus control per picture, and 9 Bento draws exactly one, on item 1's, that being the only picture the design has. **Ghost never sees this** — it is a hint the compiler resolves into the crop the theme ships with, which is why it is a control and not a data binding, why it survives a design switch, and why it declares no behaviour module and loses nothing with JavaScript off. That holds equally where the picture arrives bound: on 8, 13 and 14 at Content source: From posts the focus follows the slot, and Ghost sends nothing back about it. The fixed crops no longer decapitate portraits, and no longer lose the sides of a wide one.

**Visitor-facing strings.** A5 ships almost none: every sentence a reader sees is an authored field already. The exceptions are named — 14 Scroller's arrow labels ("Previous features" / "Next features") are theme translation-catalog strings, and the label-as-heading `tabs` emits with JavaScript off is a catalog string too, though **12 Tabs no longer contributes one**: under the ruling of 3 September 2026 the module emits its heading only where a panel carries none, and every panel here carries its own h3, so the string stays in the registry for panels that do not and is never rendered by this category. Editor advisories, placeholder copy and repeater warnings are editor chrome, never theme strings.
...
No Member visibility: no section action is drawn. Every row picture carries **Image focus** in the Image Picker popover — **the shared control, P0·9**, cited rather than restated: both axes, neither narrowed here, and Ghost never sees it.
...
No Member visibility: no section action is drawn. Item 1's picture carries **Image focus** in the Image Picker popover — **the shared control, P0·9**, cited rather than restated: both axes, neither narrowed here. It is the design's only picture, so it is the design's only focus control.
...
**Behaviour module.** `tabs`. Edit-safe: it does not run while the section is edited — the panel draws whichever item the sidebar has selected and the row follows it, which is why every frame is a resting state. **JS off:** all panels render stacked and visible, in authored order, and **the module emits no heading of its own here** — ruled 3 September 2026: *the `tabs` module emits its tab-label heading only where the panel does not already carry one*, and every panel in this design carries its own h3. **That returns the design's stated cost:** with JavaScript a reader who never touches the tabs sees one feature of six, and without it they see all six. The 160 ms cross-fade and the measured-and-held panel height go with the module — there is nothing to hold still once the panels are stacked — while the phone row's sideways scroll is `overflow-x` and survives. **The double announcement is settled, and this design's own heading was not what gave way.** The label-as-heading the registry emits used to sit above a panel that already contained an h3 repeating it, so with JavaScript off every feature was announced twice. Ruled 3 September 2026: **the module emits its heading only where the panel does not already carry one**, so here it emits nothing and each feature is announced once. **Dropping this design's h3 in the no-JavaScript branch was refused** — it would have changed the with-JavaScript state too, and the two branches should differ as little as possible, so the h3 renders in both and the no-JavaScript state differs from the other by the tabs alone. The degradation is still an acceptance criterion (FR-G4) and the deliberate h3 repeat is still this design's own rule; the ruling is general, and a panel that carries no heading of its own still gets the module's.
...
No Member visibility: no section action is drawn. Every panel picture carries **Image focus** in the Image Picker popover — **the shared control, P0·9**, cited rather than restated: both axes, neither narrowed here.
...
**Data:** Content source (Authored · From posts) with the P0·5 panel — the spotlight is the first post, the entries the rest; picture ← feature image, title ← post title, body ← excerpt, spotlight link ← the post; **How many, a number picker, 1 to 9, default six**, Ghost having no "posts per page" setting a theme can read; bound, the Items list loses Add, Remove and drag and post content is plain-text-locked. **The note naming A19 Featured and Spotlight for a post-shaped spotlight is drawn on the design itself**, beside the source control. The spotlight picture carries **Image focus** in the Image Picker popover — **the shared control, P0·9**, cited rather than restated: both axes, neither narrowed here, in both the authored and the bound state.
...
**Behaviour module.** `carousel`. Edit-safe, and nothing in the design moves on its own at any time, so the rail in the editor is the rail a reader gets. **JS off:** “The slide track is a native horizontally-scrollable `scroll-snap` strip — **fully usable**, only dots and arrow buttons are hidden.” Every card, picture and link stays reachable by scrolling; the rail keeps its `tabindex="0"` and its `aria-label`; snap is a CSS property and survives; and **tabbing to an off-screen card still scrolls it into view, that being the browser's own behaviour rather than the module's.** Two consequences. The design **offers no dots**, so the only affordance the registry sentence removes here is the arrow pair. And **Rail controls: A rule survives with JavaScript off** — ruled 3 September 2026, in the general form that matters more than this case: *a module hides only the affordances it adds, never a static element the design drew*. The rule is a 3 px track the stylesheet emits, not something `carousel` creates, so the module has nothing to hide and does not hide it. **One consequence of that is recorded rather than settled:** the fit check — the arrows and the rule are drawn only where the track does not fit the window — is a measurement, and nothing measures with the script gone, so **with JavaScript off the rule renders whether the track fits or not**. That sits against the owner's ruling of 30 August that a reader never sees a control that does nothing, which is why it is **OPEN FOR THE OWNER** in the Patch notes rather than decided here. *Flagged: reading the rule as a static element rather than an affordance is the ruling's, not this document's.*
...
## Two findings — both answered, 3 September 2026

**1 · 12 Tabs was announced twice with JavaScript off — answered.** `tabs` degrades to “All panels render stacked and visible, each preceded by its tab label as a heading.” This design's panels each contain their own h3 which **repeats the tab's label deliberately** — its own accessibility rule, written before this pass. Stacked, that gives every feature a label heading and then an h3 saying the same words. Both sides are fixed points: the degradation is an acceptance criterion (FR-G4) and the h3 repeat is the design's. **Ruled 3 September 2026, and it is the first of those two:** *the `tabs` module emits its heading only where the panel does not already carry one.* Here every panel carries one, so the module emits nothing and each feature is announced once. **The design's own h3 is never the thing dropped** — dropping it would change the with-JavaScript state as well, and the no-JavaScript branch should differ from the JavaScript branch as little as possible. Written into §12's behaviour line, the no-JavaScript table and `A5-12 Tabs`.

**2 · 14 Scroller's rule had no registry sentence — answered.** `carousel`'s degradation names dots and arrow buttons. This design **refuses dots** and offers **Rail controls: A rule** instead — a 3 px static track, 240 px wide, not draggable and not a scrollbar — so nothing in the registry says what becomes of it. **Ruled 3 September 2026, and ruled generally rather than for this case:** *a module hides only the affordances it adds, never a static element the design drew.* The rule is CSS the stylesheet emits, so **it survives with JavaScript off**; only the arrows, which `carousel` adds, are hidden. The earlier reading — an indicator hidden with the arrows — is withdrawn. **One consequence is recorded and not settled:** the fit check that withholds the arrows and the rule where the track already fits is a measurement, so with the script gone the rule renders either way. **OPEN FOR THE OWNER** in the Patch notes. Written into §14's behaviour line, the no-JavaScript table and `A5-14 Scroller`.
...
| 12 | Tabs | **all panels render stacked and visible, in authored order**, and **the module emits no heading of its own** — every panel already carries its own h3, so each feature is announced once (ruled 3 September 2026). The cross-fade and the held panel height go; the phone row's sideways scroll survives. Finding 1 is answered. |
| 13 | Spotlight | identical; at The rest: Hidden the entries are absent server-side, so nothing is downloaded and unread |
| 14 | Scroller | **the rail is fully usable** — a native horizontally-scrollable snap strip, every card, picture and link reachable, the container keeping its tab stop and label. **Only the arrow pair goes; Rail controls: A rule stays**, being CSS the stylesheet emits rather than an affordance the module adds (ruled 3 September 2026) — and it renders whether or not the track fits, nothing being able to measure without a script. Finding 2 is answered. |
...
`tabs` on **12 Tabs**; `carousel` on **14 Scroller**; **nothing at all on the other fourteen**. `core` is assumed and not listed per design. **No module name was coined in this pass**, no module was renamed, and the two registry questions in **Two findings** are answered as of 3 September 2026: `tabs` emits its tab-label heading only where a panel carries none, and a module hides only the affordances it adds. Neither ruling changed a module name, a declaration or a design's behaviour — only what each design promises with the script gone.
...
### Open questions for the architect — both answered, 3 September 2026

*Housekeeping, 3 September 2026: both came back ruled in this pass, so both are struck through with the ruling that settled each one beneath it. **Nothing in this list is open now.** The two things this pass could not settle are new, and are in the pass-five Patch notes marked **OPEN FOR THE OWNER**.*

1. ~~**12 Tabs is announced twice with JavaScript off.**~~ **Answered, 3 September 2026: the `tabs` module emits its tab-label heading only where the panel does not already carry one.** Every panel in 12 carries its own h3, so the module emits no heading here and each feature is announced once. The design's h3 is not dropped in the no-JavaScript branch — that would change the with-JavaScript state too. Written into §12, the no-JavaScript table, Two findings and `A5-12 Tabs`. No module name was coined.
2. ~~**14 Scroller's rule has no registry sentence.**~~ **Answered, 3 September 2026, in general form: a module hides only the affordances it adds, never a static element the design drew.** The rule is CSS the stylesheet emits, so it survives with JavaScript off; only the arrow pair goes. Written into §14, the no-JavaScript table, Two findings and `A5-14 Scroller`. One consequence of the ruling is new and open — see the pass-five Patch notes.
...
Every change carries the **name** of the rule or the ruling that required it. Rules are named, never numbered: the letter-and-number codes elsewhere in this project are filing labels.

**Frames changed — nine.** `A5-7 Alternating Media`, `A5-8 Media Top`, `A5-9 Bento`, `A5-12 Tabs`, `A5-13 Spotlight` and `A5-14 Scroller`: the Image focus line in each control panel now cites **P0·9** and enumerates nothing, and `A5-7`'s spec card loses the same enumeration. `A5-12 Tabs` and `A5-14 Scroller`: the no-JavaScript notice rewritten to the two registry rulings. `A5-0 Category Proof`: a **PATCH PASS FIVE** card. **No section frame was redrawn, no arrangement moved, no type scale, colour pack or spacing step changed, and no other design's panel was touched.**

### What changed, and the rule that required it

| Rule (by name) | Change |
|---|---|
| One control name means one set of values | **§0's Image fields paragraph no longer enumerates the focus values.** It names the control, names the six designs whose slots it sits under — 7, 8, 9's item 1, 12, 13 and 14 — and refers to **P0·9** for the values, the defaults, the per-slot rule and the rule for narrowing an axis. The private copy this category carried was one of the two dozen the owner ruled on. |
| One control name means one set of values | **The side-to-side axis is in force here by reference rather than by restatement.** A5's old text carried the up-and-down values only, so the axis added on 3 September 2026 arrives with the reference and needed no sentence of its own: a wide photograph cropped to 3:2 or 16:9 keeps the side the site chooses. |
| One control name means one set of values | **The six per-design lines cite P0·9 too**, so a reader who opens one design's section rather than §0 still lands on the definition rather than on a local list. None of them ever carried the values; each now says which slot the control sits under, that both axes are offered, and that Ghost never sees it. |
| One control name means one set of values | **"Ghost never sees this" is written in, once** — in §0, beside the reference. Focus is a hint the compiler resolves into the crop the theme ships with, which is why it is a control and not a data binding, and why it declares no behaviour module and loses nothing with JavaScript off. Stated for the bound state as well: on 8, 13 and 14 at Content source: From posts the focus follows the slot. |
| A design may offer fewer choices on a shared control and must say why | **No subject: A5 narrows nothing on this control, and §0 now says so with the reason.** Every picture in the category is cropped into a frame of the design's own shape — 3:2 on 7, 12 and 13, 16:9 on 8 and 14, the large tile's own box on 9 — so both axes always have work to do and there is no constraint to keep. Nothing was greyed and no value was removed. |
| A control that could never do anything here is not drawn, and the panel says why | **Checked, no subject.** P0·9's could-never case is a design that places a photograph at its own shape. A5 has none: all six picture designs crop. No never-offered note was added and none was removed. |
| The `tabs` module emits its heading only where the panel does not already carry one | **12 Tabs' no-JavaScript line is rewritten** — the registry ruling of 3 September 2026, which answers finding 1. Stacked, the panels render in authored order and **the module emits no heading here**, every panel carrying its own h3, so each feature is announced once. **The design's h3 is not dropped in the no-JavaScript branch:** that would change the with-JavaScript state too, and the two branches should differ as little as possible. Written into §12's behaviour line, §12's finding sentence, the no-JavaScript table, Two findings, the architect list and `A5-12 Tabs`. |
| A module hides only the affordances it adds, never a static element the design drew | **14 Scroller's 3 px rule survives with JavaScript off** — the registry ruling of 3 September 2026, which answers finding 2 in its general form. The rule is CSS the stylesheet emits rather than anything `carousel` creates, so only the arrow pair goes. The earlier reading — an indicator hidden with the arrows — is withdrawn. Written into §14's behaviour line, the Controls prose, the no-JavaScript table, Two findings, the architect list and `A5-14 Scroller`. |
| A5's translation-catalog list is corrected as a consequence | **12 Tabs no longer contributes a visitor-facing string.** §0 named the label-as-heading the module emits as a catalog string; under the ruling it is never emitted here, so **14 Scroller's two arrow labels are the only theme strings A5 ships**. The catalog string itself stays in the registry for panels that carry no heading of their own. |
| Printed design totals stay out of product copy | **Honoured, and nothing written.** No design total — old, corrected or computed — appears in anything authored in this pass. Where this document counts, it counts this category's own sixteen designs and says so. |
| The marketing and app screens print no design total | **Not this category's, and not touched.** A5 holds no marketing or app screen. |
...
| The roster, with [Free] marked | **Unchanged and re-checked.** The `**[Free] designs:**` line stays at the head of this document, written once, naming 1 Three Up and 6 Rows. |
| Nothing was renumbered | **Sixteen designs, 1 to 16, no gaps and no number reused.** No design was added, deleted or renamed; no module name was coined; no control was renamed; no control gained or lost a value. |

### Open items — recorded, not guessed

- **OPEN FOR THE OWNER — with JavaScript off, does 14 Scroller's rule render even where the track already fits?** What I would have needed to know: which of two rulings wins when they meet. *A module hides only the affordances it adds* keeps the rule with the script gone; the design's own fit rule — arrows and rule drawn only where the track does not fit — is a measurement, and nothing measures without a script, so the rule must render either way; but the owner's ruling of 30 August says a reader never sees a control that does nothing. What I did instead: applied the work list — the rule survives, as ruled — and wrote the consequence into §14 and the no-JavaScript table in plain words rather than inventing a CSS-only fit test. If the answer is that it should not render, the fix is a rule that is drawn only at Rail controls: A rule *and* a track wider than the window, which no one can write in CSS today.
- **OPEN FOR THE OWNER — is a picture Ghost supplied entitled to an Image focus?** What I would have needed to know: whether focus follows the slot regardless of who supplied the image. P0·9's test is whether the design crops and says nothing about the image's source, and A5 offers focus in both the authored and the bound state on 8, 13 and 14 — while A1 and A3 withhold it from post feature images on the grounds that the image is Ghost's. What I did instead: left A5's behaviour exactly as found — offered in both modes — cited P0·9 for it, and recorded the divergence here. A2 raised the same question independently, which is the sort of thing this note exists to surface.
...
**The two repository-maintained pieces were not touched.** No count of designs was written into any copy in this pass, and none was removed or corrected — nothing in A5 carries one. **P0's per-prop mark allowlist was not re-authored, paraphrased or summarised:** A5 cites the P0·1 toolbar by name and never restates what marks a field permits. Where it was unclear whether a line came from the repository or from this project, the line was left as found.

**Everything else in this document stands as written.** Responsive rules, content fields, controls, data bindings, empty states, accessibility notes, the structural descriptors, the roster, the hand-offs and all four earlier passes' Patch notes are unchanged. The two behaviour declarations are unchanged — `tabs` on 12, `carousel` on 14 — and both rulings this pass applied changed what a design *promises with the script gone*, never what it runs.
