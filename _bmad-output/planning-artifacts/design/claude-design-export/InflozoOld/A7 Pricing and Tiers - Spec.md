...
**Design patch pass — 29 August 2026 (this document's current state).** Five rulings landed on this category and one of them reaches every design in it. **Ghost's templates cannot add**, so the saving line belongs to the billing switch alone — the switch's own script computes it, and with JavaScript off it does not render — and it is gone from 15 Both Prices, which carries no switch. **2 Table's matrix is literal:** one row per tier-and-benefit pair, ticked in that tier's column and nowhere else; the de-duplication of shared benefits and the reading of “Everything in Free” as inheritance are both withdrawn, because a template can neither compare two tiers' lists nor count them. **The comparison table keeps scrolling sideways on a phone** — CSS cannot transpose a grid — and 11 Free and Paid's phone reorder is **visual only**, its source order fixed. **13 Members loses the join date:** Ghost gives a theme none, so a free member's card carries no line at all. **Every paid action renders only where a payment provider is connected, and Portal is JavaScript** — the claim that all fifteen designs are “subscribable with JavaScript off” is withdrawn. And **no design turns into another design**: every hand-off in this document is now advice the panel gives. **Nothing was renumbered.**

**[Free] designs:** 1 Cards · 12 Ledger

*(Shortlisted in this pass and **confirmed by the owner**: Cards and Ledger — the loudest and the quietest. Ledger writes no headline of its own, so a free site carries that in the section above it.)*

**Specification-only pass, an earlier session.** Every design now carries four added fields — descriptor, structural descriptor, archetype, behaviour module — at the head of its section. Nothing drawn changed and no earlier field was rewritten. **One correction is in scope and has been made:** the two module names this document coined, `M-cadence` and `M-tabs`, are not in the fixed 31-module registry (FR-G7) and map to nothing the build can compile. They are renamed to the registry modules they actually are — `price-toggle` and `tabs` — and each design's no-JS sentence is now the registry's own, quoted, rather than one composed here. **The frames and spec cards carried the coined names, and 10 Tabs' panel carried the no-JS claim that pass overturned; on module names and no-JS degradation the registry is the authority rather than the drawn panel. That drawing debt is paid in the fourth pass below.**
...
**No-JS baseline, corrected in the patch pass.** Prices, benefits, badges, planes and actions are all server-rendered, so **all fifteen designs render complete with JavaScript off** — 10 Tabs included, its module degrading rather than failing. **They are not subscribable without it.** Every action is an `<a>` to a Portal fragment and **Portal is JavaScript**: with JavaScript off the link changes the URL and nothing opens. The earlier claim that A7 renders “complete and subscribable with JavaScript off” is withdrawn here, on every frame, and in every section below; each design's panel carries the line, and **A7 contains no subscribe or sign-in form of its own**, so there is no form for a no-JavaScript notice to replace — the notice's subject is Portal, and the sentence is the panel's. Each module's degradation is the registry's and is an acceptance criterion (FR-G4): `price-toggle` — “Both monthly and yearly prices render side by side, each labelled — no toggle control shown.” `tabs` — “All panels render stacked and visible, each preceded by its tab label as a heading.” **The consequence for A7 is worth naming, because it is not what this document assumed.** The switch's no-JS branch is 15 Both Prices' arrangement, reached by degradation rather than by choice: the fourteen designs carrying the switch must server-render both of a tier's prices, each labelled in words, and the module reveals one of them. Where a design has no room for two prices side by side the pair stacks — 2 Table's column head, 8 Slim Row's line, 9 Big Price's number, 12 Ledger's price column — both still drawn and both still labelled. The action's `<a>` carries the cadence the page was rendered with, monthly, which is now one of two visible prices rather than the only one, and each design states that cost rather than papering over it. At Billing period Monthly or Yearly there is no module and nothing to degrade.

**Repeating items.** The tiers come from `{{#get "tiers"}}` and the site does not author them: **no design in A7 offers an Add or Remove control**, the order is Ghost's own price-ascending at every width and in every state (11 Free and Paid's visual-only phone reorder excepted, stated there), and the per-design counts, the fewer-than-expected states — including exactly one and exactly zero — and the missing-field behaviour are in §0 and in each design's Empty rule, unchanged by this pass.
...
**The tier count is data, not a control.** Three cards of 416 on a 24 px gutter, two of 632 on 32, one of 480 centred — the theme picks by counting. **The order is Ghost's own, price ascending, at every width and in every state**; the recommended tier is never moved to the front, and only 11 Free and Paid reorders, on a phone, visually and in CSS alone, stating it. **Four or more tiers**: 3 Stack, 12 Ledger and 4 Split Head take them as ordinary rows, 2 Table scrolls, and **every columned design keeps its own arrangement — the cards or panels wrap — while the panel advises one of those three. Advice, never a switch.**
...
**Responsive floor.** Three tiers hold above 1081; **at 1080 and below they become their own two-line rows** — name, price and description left, action right on 168, benefits under a hairline in two columns. That row is the shape 3 Stack draws at every width, borrowed as a layout and **never as a change of design**. Two tiers keep two columns to 768; one keeps its card. At ≤ 767 one column at 16 px in price order, card padding 24, price 36, **actions 48 px at full card width**, the switch full width with the saving line beneath it.
...
Three tier cards in one row under a centred head. The floor the other fourteen depart from, and the design every panel's advice names.

**Descriptor.** The category's floor: three equal cards on the page ground with nothing added to them, and the arrangement nine other panels advise rather than inventing one of their own.
...
**Responsive.** Three tiers hold above 1081; at 1080 and below the cards become this design's own two-line rows — the shape 3 Stack draws at every width, borrowed as a layout. Padding 96 → 80, card 32 → 28, title 40 → 34, price 48 → 34, rows 16 apart. ≤ 767 as §0.

**Empty.** As §0. Four or more tiers → the cards wrap to a second line and the panel advises 3 Stack.
...
**Descriptor.** The only real `<table>` in A7, the only design that draws a benefit matrix at all, the only one that scrolls — at every width below 1081, the phone included — and the only one that takes a fourth tier.
...
**Responsive.** Above 1081 the table fits the content width. **At 1080 and below it scrolls sideways** — 260 for the pinned name column, 200 a tier — in a `tabindex="0"` container with an accessible name, a visible focus ring and a fade at the right edge; **sticky is a position, not a behaviour**. **At ≤ 767 the table stays a table and keeps scrolling sideways** — 150 for the pinned name column, 130 a tier, cell padding 11, rows 13.5 px, the fade at the right edge. **It does not transpose into cards: CSS cannot**, and a second set of markup would be a second design. The panel advises 1 Cards for a site that would rather not scroll.
...
**Empty.** No benefits anywhere → the rows go and the head stands alone with its prices and actions; the panel advises 1 Cards or 8 Slim Row. One tier → one column of ticks, which is a list drawn as a grid; the panel advises 6 Single Tier.
...
**Flagged.** The literal one-row-per-pair matrix and its repeated benefit lines, dropping the description, the continuous column plane, the tick's muted colour, the two row-mark values and the blank cell's silence, Rows Tinted dropping the hairlines, the dark tint stepping down, the pinned-column scroll at every width below 1081 including the phone, and this being the one design that takes a fourth tier.

---

## 3. Stack

One full-width row per tier: name and price at the left, benefits in the middle, action at the right. **The only arrangement in A7 that does not care how many tiers there are**, which is why its two-line row is the shape the columned designs borrow for their own tablet width.

**Descriptor.** The only arrangement in A7 that does not care how many tiers there are, which is why its two-line row is the layout the columned designs borrow at 1080 instead of inventing one of their own.
...
**Responsive.** At 1080 and below the head goes above the tiers on a 600 px title measure with 3 Stack's rows beneath; the switch keeps its place in the head and puts its saving line back beside it; the note ends the section. At ≤ 767 it is one column of its own with the switch under the head — the shared phone arrangement, borrowed as a layout.

**Empty.** As 1. **No sub and no switch → the head is an eyebrow, a title and a note**, and the panel advises 1 Cards, whose centred head does not leave 440 px half empty.
...
**Responsive.** At 1080 and below the panels become two-line rows on the band — the shared tablet layout; band padding 56, inner 40, page ground above and below 40. At ≤ 767 always full bleed, band padding 44/20, panels 24, page ground 24.
...
**a11y.** The band is **not a landmark, region or theme boundary** — a background colour on a named section. Benefit text stays at full strength while marks and descriptions are muted: nine muted lines on an inverted panel is a paragraph nobody reads. The outlined border is 2.4:1 and carries nothing; the label carries it. **Forced colours drop the band and both mixes**, and the section keeps its own arrangement in system colours.
...
One card at 480, centred, with the free tier beside it. **Settlement 4 built rather than ruled**, and the only design in A7 that advises upwards.

**Descriptor.** The only design built for exactly one paid tier, the only one that sets a planed tier beside a tier with no plane at all, and the only one that advises upwards.
...
**Empty.** No benefits → the card ends at its action and the free tier's own list is drawn instead. **No free tier** — a site can disable it — → the paid card alone, the control unavailable, the setting named. **Three paid tiers → the cards stack in its own 480 column and the panel advises 1 Cards.**
...
**Flagged.** One paid tier being two cards, the 480 + 320 division and the planeless free column, Display 56 as this design's fixed step, the two-column benefits inside a card, the free tier's three positions and the fixed sentence, dropping the recommendation, the vertically centred free column, the phone's forced Under it, and the advice up to 1 Cards.
...
**Responsive.** At 1080 and below the large card goes full width with its internal columns intact and the small cards sit side by side beneath it at 361 each; **this design keeps its own arrangement at every width** — equal rows would delete it. Price 56 → 44 with the small tiers unchanged at 34, which narrows the gap and is the honest cost of the collapse. At ≤ 767 one column in price order, the large tier still larger by padding, price and plane.

**Empty.** No recommendation → equal cards, and the panel advises 1 Cards. No benefits on the recommended tier → the large card loses its right column and becomes a single column at 848.
...
**Flagged.** Requiring a recommendation, the two divisions, the large card's internal columns and vertical hairline, Display 56 and 34, the one-step-down scale that removes nothing, Benefits hidden being symmetric, the 42 px floor, refusing the equal-row collapse, and the tablet's narrowed price gap.
...
**Empty.** No note → the action alone. One paid tier and no free tier → one name and one price, and the panel advises A6·10.
...
**One or two paid tiers only.** Two stack, each with its own price block, separated by a rule, the recommended one keeping the accent and **the badge doing the plane's work, since there is no plane.** Three or more → the price blocks keep stacking and the panel advises 1 Cards: two 112 px numbers on a page is a poster, three is a chart.
...
**Empty.** One tier → the tab row is hidden and the single panel stands alone; the panel advises 6 Single Tier. Four or five tiers → the row wraps to a second line, a supported state.
...
**Responsive.** The halves hold at 834 at 365 each with **the divider dropped** — the card's own edge is 12 px from where the hairline would be, and two lines 12 px apart is one line drawn twice. **At ≤ 767 the paid tier comes first visually** — the one design in A7 that reorders its tiers — **and the reorder is visual only: CSS order, with the source order unchanged**, since CSS cannot reorder the DOM at a breakpoint. A screen reader and the keyboard still meet the free tier first, and that cost is stated rather than papered over. A horizontal hairline sits between them.

**Empty.** No benefits on the free tier → the halves are honestly unequal. **No recommendation → the paid tier keeps its card and its accent**; only the badge goes, because the plane here says “this is the offer” rather than “this is recommended”. **A second paid tier → the paid half holds both, one under another, and the panel advises 1 Cards; the free tier turned off → the free half is hidden and the paid card centres, and the panel advises 6 Single Tier** — both advised in the panel before they happen, and neither a switch.
...
**Flagged.** The plane as the only difference, both halves keeping every fact, the free half's action position, the full-height divider and its drop at 834, the two divisions, the two free-action values, the accent staying with no recommendation, the phone's visual-only reorder, and both advised states.
...
**Flagged.** Reading which tier a reader is on, the renewal date being the only member fact drawn and the free member's card carrying no line, the action's replacement by a date line, the outlined badge and its precedence, the computed accent and its disappearance at the top tier, “Upgrade” versus “Change plan”, treating cancelled as visitor, the three-controls-one-destination cost, and the tablet's shortened date.
...
**Empty.** **No assurances → no strip and no hairline**, and the `note` returns in its place; the panel says so at the field rather than drawing an empty rule, and advises 1 Cards.
...
**Per-card, not per-section.** A tier with no yearly price loses its second line and leads with monthly **on that card alone** — the opposite of the switch's rule, because a control must be true of everything it controls while a line of text need only be true of its own card. **Nothing about a saving is drawn at any value**, and the theme never says “no discount”.
...
**Responsive.** At 1080 and below the second line joins the price's baseline whatever Second price says, and **nothing about the line changes with width** — it is two of Ghost's own strings, with no computed copy to shorten. At ≤ 767 it goes back under the price at every value and Second price is ignored; **this is the design's best width.**
...
**Flagged.** The muted second-cadence line and its refusal of any saving, the free tier's fixed line, the per-card fallback, the three second-price arrangements and the advice against Two prices, the Portal link following the lead cadence, refusing a strikethrough or a percentage, and the tablet's shortened saving.
...
## Patch notes — pricing and tiers patch, 29 August 2026

Every change below carries the **name** of the rule that required it. Rules are named, never
numbered. Where a ruling could not be applied without inventing a decision, it is written here as an
**open question** and asked in plain words at the end.

**Frames changed — all sixteen:** `A7-0 Category Proof` and `A7-1` … `A7-15`.

### This category's own rulings

| What changed | Why, and where |
|---|---|
| **The saving line belongs to the billing switch alone, and it needs JavaScript.** Ghost's templates cannot add, so the whole-month “Two months free on yearly” is the `price-toggle` module's arithmetic — not the template's. Every panel in the fourteen switched designs now says so, and says that with JavaScript off there is no switch and no saving line. | *(This category's own ruling; Ghost's templates cannot count, add or remember.)* |
| **15 Both Prices draws no saving at all.** Its **Saving line (Show · Hide)** control is deleted — five controls now, not six — the “· two months free on yearly” half of the muted line is gone from every frame, light, dark, tablet and both phones, and the tablet's “shortens to two months free” ruling goes with it. Nothing is struck through and no percentage is shown, as before. | *(This category's own ruling; the same Ghost fact.)* |
| **2 Table's comparison matrix is literal: one row per tier-and-benefit pair.** The de-duplication of shared benefits and the “everything in the tier below” inheritance parsing are both withdrawn — a template can neither compare two tiers' lists nor count them. Rows are grouped in Ghost's tier order and ticked in one column only; the sample content is eleven rows rather than nine, “Everything in Free” is an ordinary row of Reader's, and a benefit two tiers both claim is drawn twice. Redrawn in the desktop light, dark, tablet and annotated frames. | *(This category's own ruling; Ghost's templates cannot count, add or remember.)* |
| **On a phone the comparison table keeps scrolling sideways.** Both 390 frames were the hand-off to cards and are redrawn as the scrolling table — 150 for the pinned name column, 130 a tier, cell padding 11, rows 13.5 px, the fade at the right edge, the container focusable and named. CSS cannot transpose a grid. | *(This category's own ruling; CSS cannot see content — and no design ever turns into another design.)* |
| **11 Free and Paid's phone reorder is visual only.** The claim that “the DOM order changes with it, so reading and tab order still match” is withdrawn: the reorder is CSS `order`, the source order is fixed, and the panel states that a screen reader and the keyboard still meet the free tier first. | *(This category's own ruling; CSS cannot see content.)* |
| **13 Members loses the join date.** Ghost's member object gives a theme no join date, so it would render as today on every live site. “Joined 12 March” is deleted from the free-member state and the dark frame; **a free member's card now carries no line at all** and closes up, while a paid member keeps the renewal line between its two hairlines. The data list, the fields note and the flagged list follow. | *(This category's own ruling; some fields we drew do not exist.)* |
| **Every paid action is conditional on a connected payment provider**, stated in the Data group of all fifteen designs beside the existing `@site.paid_members_enabled` gate. | *(Member buttons are conditional, and Ghost's sign-up pop-up needs JavaScript.)* |

### The library-wide rules

| Rule | What it did here |
|---|---|
| **Member buttons are conditional, and Ghost's sign-up pop-up needs JavaScript** | **The biggest correction in the pass.** A7's actions are all `<a>` links to Portal fragments, and Portal is JavaScript — so the category's claim that “all fifteen designs render complete **and subscribable** with JavaScript off” was wrong. It is withdrawn in §0, in every design's Data group and in the category proof: the sections render complete, and **with JavaScript off these links do nothing**. Every paid action also carries the payment-provider condition. |
| **The no-JavaScript notice** | **No subject, and nothing to invent.** A7 contains no subscribe or sign-in form of its own — Portal owns every field — so there is no form for a notice to replace. What was wrong here was the promise, not a missing notice, and the promise is withdrawn above. Every design's no-JavaScript line is listed below. |
| **No design ever turns into another design** | **Nineteen phrases deleted across fifteen frames and this document.** 1 Cards' “four or more tiers → 3 Stack, named” and its “1080 hand-off”; 2 Table's phone hand-off to cards, its one-tier and no-benefits hand-offs; 3 Stack's “every columned design hands off to it”; 4 Split Head's phone “is 1 Cards' single column”; 5 Contrast Band's “becomes 1 Cards in system colours” and “four or more hands off”; 6 Single Tier's “at three tiers it becomes 1 Cards”; 7 Highlight's “the design is 1 Cards” with no recommendation, and its two “does not hand off to 3 Stack” lines; 9 Big Price's “three or more hands off”; 10 Tabs' “the design is 6 Single Tier”; 11 Free and Paid's two end states; 14 Assurances' “the design is 1 Cards with the note restored”. **In every case the section now renders itself** — the cards wrap, the panels wrap, the paid cards stack, the tab row hides, the strip and its hairline go, the free half is hidden — **and the panel advises another design.** Advice, never a switch. Where a design borrows a two-line row or a single column at a narrow width, that is its own layout and is now worded as one. |
| **The two free designs are the owner's choice — ask him** | Shortlisted the five plainest designs a site could ship unfinished-free, none of which needs photography (A7 draws no imagery at all): **1 Cards · 3 Stack · 12 Ledger · 8 Slim Row · 6 Single Tier**. **The owner chose 1 Cards · 12 Ledger** — the loudest and the quietest — and the line is recorded at the head of this document. **[Free]** badges are drawn on the roster in `A7-0`. **Ledger has no headline of its own**, so a free site writes that in the section above it; the panel says so at the design. **Closed.** |
| **Avatars with no photograph** | **No subject.** A7 renders no person: no author, no member avatar, no initials anywhere in fifteen designs, and 13 Members refuses the avatar it could have read. |
| **The remove button never greys out** | **One subject: 14 Assurances' `reasons[]`.** Already compliant and unchanged — the P0·3 item controls state that Remove is never disabled, and the frame draws it that way. The Ghost-sourced tier list has no Add and no Remove on any design, so there is nothing else to check. |
| **Slider labels** | **No subject.** A7 draws no slider. Every control is a named-value row whose title says what it affects — Billing period, Title size, Row padding, Card padding, Row height, Band edges, Price scale. |
| **Gap names are “Tight · Normal · Loose”** | **No subject.** A7 has no gap control; the spacing rows are padding ladders and read Compact · Comfortable · Spacious. No Tight/Even/Airy or Tight/Standard/Wide existed anywhere in the category. |
| **Item counts are a number picker** | **No subject.** The tier count is data, not a control — no design offers a count row, an Add or a Remove — and 14's `reasons[]` is an authored list with item controls rather than a count. |
| **A design may offer fewer choices on a shared control, and must say why** | Re-checked on all fifteen. **There is no “Inherit” value anywhere in A7.** The colour swatch row is not drawn in this category. Background role is disabled at Contrast on fourteen designs with the reason shown, kept as a value on 8 Slim Row, and locked at Contrast on 5 Contrast Band; Vertical spacing is locked on 3 Stack; Top divider locks None on 5, 12 and on 8 at Rules Above and below. Every lock draws its reason. |
...
Every design's line has two halves: what the module does without script, and what Portal does not do.
**The second half is the same sentence on all fifteen:** *the actions are links into Portal, and with
JavaScript off nothing opens — the section reads but cannot be subscribed from.*

| # | Design | Module | Without JavaScript |
|---|---|---|---|
| 1 | Cards | `price-toggle` | Both prices side by side on every card, each labelled; no switch, no saving line. |
| 2 | Table | `price-toggle` | Both prices stacked in each column head, each labelled; no switch, no saving line. The matrix, the scroll and the pinned column are CSS and keep working. |
| 3 | Stack | `price-toggle` | Both prices on two lines in the 300 px name column; no switch, no saving line. Every row renders at any tier count. |
| 4 | Split Head | `price-toggle` | No switch in the head — so the head loses the first tab stop this design gives it — and no saving line; both columns, the divider and every row render. |
| 5 | Contrast Band | `price-toggle` | Both prices on each panel; no switch, no saving line. The band, both panel styles, the inverted badge and the 45 % outlines are colour, not script. |
| 6 | Single Tier | `price-toggle` | The one paid card has room to draw the pair literally side by side; the free tier keeps the word “Free”. No switch, no saving line. |
| 7 | Highlight | `price-toggle` | Both prices in the large card's left column and in each small card; the division, the internal columns and the hairline are CSS. |
| 8 | Slim Row | `price-toggle` | Three names each carrying two prices will not hold one line, so at Billing period Both the row takes Prices Under the names whatever the control says, and wraps. Names, rules, section action and note render complete. |
| 9 | Big Price | `price-toggle` | Two 112 px numbers cannot share a line, so the second price takes a 17 px muted line under the first with its cadence in words. The measured 720 px drop is layout, not script. |
| 10 | Tabs | `tabs` + `price-toggle` | All three panels render stacked and visible, each under its tab label as a heading; both prices per panel, no switch, no saving line. |
| 11 | Free and Paid | `price-toggle` | Only the paid half has a pair to draw; the free half keeps the word “Free”. The divider, both actions and the visual-only phone reorder are CSS. |
| 12 | Ledger | `price-toggle` | The fixed 170 px column takes the pair on two lines, still right-aligned; hairline rows, middot line and every Actions value render unchanged. |
| 13 | Members | `price-toggle` | The member state is server-rendered, so the plane step, the missing action, the renewal line, the outlined “Your plan” badge and the computed accent all survive. |
| 14 | Assurances | `price-toggle` | The three cards and the strip render complete; the 2 + 1 tablet grid and the phone's forced stack are media queries. |
| 15 | Both Prices | **none** | Identical with script on or off. It has no module to degrade — and, since the patch, no saving line either. |

### Open questions

**Both questions this pass raised have been answered by the owner, and nothing is left open.**

1. **Which two designs are free** — **answered: 1 Cards · 12 Ledger.** Recorded at the head of this
   document and badged on the roster in `A7-0`.
2. **What a free member's card shows where the join date was** — **answered: nothing.** The card ends
   after its benefits: the plane step and the outlined “Your plan” badge already tell the reader which
   tier is theirs, so no line is invented and nothing can go stale. Drawn that way in 13 Members.
3. **Nothing else in this pass required a decision nobody has made.** No registry addition was needed:
   A7 still declares exactly `price-toggle` and `tabs`, and no module name was coined.
