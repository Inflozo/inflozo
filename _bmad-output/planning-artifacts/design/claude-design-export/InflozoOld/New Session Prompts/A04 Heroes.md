# Inflozo — Design Prompt 3: A4 Heroes
...
|---|---|---|---|
| A4 | Heroes | 18 | The flagship, and the pilot for control density. |
...
8. **No design ever turns into another design.** A placed design is the design that renders: it hides what does not apply (a sideways-scrolling row whose track fits shows no arrows and no fade), and the panel may **advise** — "at this count, 1 Row reads better" — never switch. Hand-off, authorised by this rule until the design patch pass, is **retired library-wide** (patch Part A·A8, ruled to apply in all 33 categories). The clause that authorised hand-off is **struck**, and one line stands in its place: a placed design is the design that renders; it hides what does not fit. **It still adapts, by four permitted moves only** — reflow (columns stack, a row wraps), hide (a part that cannot fit goes and its neighbours close up), scroll (a row becomes a swipeable track, arrows and fade only when it overflows) and collapse (a nav folds into a menu button, panels into an accordion). All four keep the design's own type, colour, spacing and claim. **Responsiveness is a requirement, not a control:** every design says in words what it does at 834 and 390 and is framed at both, and a design that cannot hold together at 390 is redrawn with its own narrow arrangement or cut and its number retired — never served as a different design.

---

## 8. What A4 must settle

1. Where A1·4 Overlay lands on each hero — every image hero must tolerate a transparent header above it.
2. Image, no-image and video-poster variants, and which fields each needs.
3. How the eyebrow / headline / sub / actions set scales from 1440 to 390 without dropping the actions.
4. What a hero renders on a page with no feature image, and on templates other than home.
