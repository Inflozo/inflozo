...
3. **A12's finding 4 recurs unchanged: 8 Rail's precondition is a measurement and no module measures.** The hand-off — if the cards are not wider than the content width, draw 12 Media Top — needs the track's rendered width. With JavaScript off the section stays a rail whose track happens not to overflow; **it does not become a grid, because a grid is a cell layout and nothing without script can decide to build one.** Carried from A12 rather than re-litigated.
4. **A13·14 Index was the design a site would ask to extend, and half of that finding is now answered.** `duration` — text, ≤ 12, optional — **joins `steps[]` in the controls pass**, drawn as 14 Index's third Columns value and 3 Rows' end slot and kept invisible in the other thirteen, on the reasoning the old finding stated: A13's shared list is where such a field lands, and it is cheaper to add before more categories ossify the list than after. **`status` stays refused** — there is no honest way to render a state on a marketing page, and a process that tracks its own progress is an application. **What remains for the architect is the timing rule rather than the field:** adding to a shared list after a category is drawn changes every design's “kept and not drawn” line, and it did here — thirteen designs gained a held field in one edit.
...
**Member Visibility lands nowhere in A13 and is recorded rather than added.** The category bears no call to action: its one destination is a text link at the section-link treatment, 13 Slim Bar draws no link at all at Lead None, and a step is never a target. *Flagged: if the architect wants the control universal rather than CTA-scoped, all fifteen gain it at once.*
...
**Remove is never disabled, and a floor hands off rather than blocking the edit.** **Every step after the removed one re-numbers**, which is A13's one destructive-looking edit that is not destructive: nothing was authored, so nothing is lost. Below a design's designed floor it hands off to the design its panel names — **14 Index in four cases, 1 Three Up in four, 3 Rows in four, 2 Track in one**. **At one step the section does not render and the panel says a process needs two**; at zero the repeater shows Add step and a line saying so. *Flagged: the hand-off-instead-of-block rule is A11's, carried unchanged.*
...
Three to six steps in equal cells, three or four across, each a numeral above a title above a paragraph. The design a site gets when it types “how it works”, and **the design five others hand off to**.

**Descriptor.** The category's floor — a numeral above a title above a paragraph in equal cells, with no container, no picture, no label and no behaviour: the arrangement five other designs hand off to rather than inventing one of their own.
...
- **Remove.** Never disabled; everything after it re-numbers. Down to two → two cells at the count's width; **down to one → the section does not render**, and the panel says a process needs two.
- **Reorder.** Meaningful and consequential: it re-numbers, and the numerals are what hold two rows together as one sequence.
- **Counts.** **Two to six, all drawn**, at three or four per row. **Seven or eight → 14 Index**, named in the panel before the count is reached.
...
- **Add.** Lands last and extends the track; **a sixth add changes the design** and the panel reads “6 steps — 1 Three Up draws all six” before the edit.
- **Remove.** Never disabled; the track re-spaces and everything after re-numbers. **Below three → 1 Three Up's two cells**; at one the section does not render.
- **Reorder.** Meaningful. **A marker's figure moves with its text because they are one `<li>`** — the design's most important build note.
- **Counts.** **Three to five, and the design is designed for exactly those.** **Six or more → 1 Three Up**: a track that wraps is not a track, because the connector would have to run off one line and start again on the next.
...
- **Remove.** Never disabled; the rows close up, the rule redraws in the new gaps, and everything after re-numbers. At one the section does not render.
...
- **Counts.** **Two to six** at Three or Four per row; Count Five is not offered and the measure is the reason. **Seven or eight → 14 Index**, which drops the cards and the icons and says so.
- **Zero.** The section does not render.
...
**Data** · **Source: Authored · From posts.** Authored is the default and every frame's state. At From posts it is **P0·5 configured for reading paths** — Filter **By tag**, Order **fixed at Oldest first**, Count **≤ 6**, this design's own ceiling — with `title` ← post title, `body` ← custom excerpt, `image` ← feature image, **each bound step linking to its post**, and Add, Remove and drag hidden: **P0·3's read-only card**. `label`, `duration` and `icon` have no Ghost source and stay empty.  0 and 1 → does not render. 2 → two cards at the count's width. 3 to 6 → drawn. **7 or 8 → 14 Index.** **In bound mode every box carries the numeral plate and the top-right figure is dropped for that card**, exactly as at an authored empty icon.
...
- **Remove.** Never disabled; **the head column is unaffected**, so a short list leaves the prose taller than the sequence it introduces. At one the section does not render.
- **Reorder.** Meaningful; the first step sits level with the top of the title because both columns are top-aligned.
- **Counts.** **Two to six** at One or Two per row; five at Two is 2 + 2 + 1 and the panel names the counts that divide rather than disabling the value. **Seven or eight → 3 Rows**, which has the full width for them.
...
**Fields** · every section field except `image` and `imageAlt`, **including `body` ≤ 900 and ≤ 3 paragraphs**; two to six steps reading `title` and `body`. **`title` is required in practice** — with no title the section hands off to 1 Three Up.
...
**Behaviour module.** **none.** The band is a `background-color` on the section and the derived muted and hairline are CSS mixes of one token pair. **JS off:** identical, and print and forced colours drop the band and draw 1 Three Up with or without script either way.

**Items** · `steps[]`, two to six, on the band. **Identical to 1 Three Up's in every particular, which is the point of the pair.**

- **Add · Remove · Reorder · Counts · Zero.** As 1 Three Up, without exception: lands last, never disabled, authored order, two to six drawn, one → does not render, seven or eight → 14 Index.
...
**Data** · as 1 Three Up, including **Source: Authored · From posts.** Authored is the default and every frame's state. At From posts it is **P0·5 configured for reading paths** — Filter **By tag**, Order **fixed at Oldest first**, Count **≤ 6**, this design's own ceiling — with `title` ← post title, `body` ← custom excerpt, `image` ← feature image, **each bound step linking to its post**, and Add, Remove and drag hidden: **P0·3's read-only card**. `label`, `duration` and `icon` have no Ghost source and stay empty.  At seven or eight the bound section hands off to 14 Index, which draws on the page ground.

**Empty** · as 1 Three Up. **Forced colours and print both drop the band and the section becomes 1 Three Up.**
...
- **Counts.** **Two to four**, and the ceiling is low for a stated reason: four steps at Even and 3:2 are 1,940 px of section before the head; five are 2,425. **Five or more → 3 Rows**, which keeps the bodies and drops the pictures.
- **Zero.** The section does not render.
...
**Descriptor.** The only design the reader moves — a full-bleed snap track of equal step cards wider than the window — and the only one in A13 whose existence is decided by a measurement at render rather than by what was authored.

**Structural descriptor.** `carousel · none · page · variable · top · reader-scrolled step track`

**Archetype.** carousel.

**Behaviour module.** `carousel`. **Edit-safe** — it does not run while editing and the scroll position is never persisted or restored, which is why every frame is drawn at rest. **JS off:** “The slide track is a native horizontally-scrollable `scroll-snap` strip — **fully usable**, only dots and arrow buttons are hidden.” **A13·8 draws that branch as its own drawing:** the snap is on, there are no dots to hide, and JavaScript adds only the two arrows and the two fades. **A12's finding 3 is settled here in the registry's favour** — its cards are equal, the steps are ordered, and a step is exactly the unit a reader wants to land on. **The measured precondition has no branch in any module**, so with JS off the section stays a rail whose track happens not to overflow rather than becoming 12 Media Top: a grid is a cell layout, and nothing without script can decide to build one. Reduced motion keeps the scroll and the snap and drops the smooth animation.

**Items** · `steps[]`, two to eight, one card each on the track.

- **Add.** Lands last at the right end of the track — **outside the window, and the rail does not scroll to show it**; the repeater's row confirms the addition. Adding is also the edit that can bring this design into existence: **a fourth step at Standard turns a grid that fits into a rail.**
- **Remove.** Never disabled. **Removing until the cards fit hands off to 12 Media Top at that width and leaves it a rail at narrower ones** — the same list, two designs, decided by the window.
- **Reorder.** Meaningful; the track is authored order, it re-numbers, and the snap points move with the cards.
- **Counts.** **Two to eight**, and the count class is `variable` because **the design has no designed count**: a track absorbs any number and the precondition is a width, not a total. **This design absorbs the eight-step ceiling.** Everything fits → 12 Media Top, named.
...
**Data** · **Source: Authored · From posts.** Authored is the default and every frame's state. At From posts it is **P0·5 configured for reading paths** — Filter **By tag**, Order **fixed at Oldest first**, Count **≤ 8**, this design's own ceiling — with `title` ← post title, `body` ← custom excerpt, `image` ← feature image, **each bound step linking to its post**, and Add, Remove and drag hidden: **P0·3's read-only card**. `label`, `duration` and `icon` have no Ghost source and stay empty.  0 and 1 → does not render. **Everything fits the content width → 12 Media Top.** 2 to 8 with an overflowing track → the rail. Over eight the repeater stops. **The precondition is unchanged in bound mode** — it is still a measurement, and no module makes it.
...
**Flagged** · **accepting snap where A11·9 and A12·13 refused it**; the 306 and 380 card widths; the 14 px picture gap; one card plus one gutter per click; Crop None holding authored pictures; Head Centred unavailable at Arrows Above the rail; printing as a wrapped grid.
...
- **Add.** Lands last; **a fourth add disables Display 104** and the panel says so before the edit.
...
- **Counts.** **Two to four.** Two draws two 636 cells with the body on a 480 measure; four draws 306 cells at Large 72 only. **Five or more → 3 Rows.**
- **Zero.** The section does not render.
...
**Data** · **Source: Authored · From posts.** Authored is the default and every frame's state. At From posts it is **P0·5 configured for reading paths** — Filter **By tag**, Order **fixed at Oldest first**, Count **≤ 4**, this design's own ceiling — with `title` ← post title, `body` ← custom excerpt, `image` ← feature image, **each bound step linking to its post**, and Add, Remove and drag hidden: **P0·3's read-only card**. `label`, `duration` and `icon` have no Ghost source and stay empty.  0 and 1 → does not render. 2 to 4 → drawn. **5 or more → 3 Rows.**
...
- **Counts.** **Two to six** at Stacked, **four to six** at Two columns, which is disabled below four. Six divides; five at Two columns is 3 + 2 with the left column taller, stated rather than corrected. **Seven or eight → 14 Index**, which is this design without the panel and with a tighter row.
- **Zero.** The section does not render.
...
**Data** · **Source: Authored · From posts.** Authored is the default and every frame's state. At From posts it is **P0·5 configured for reading paths** — Filter **By tag**, Order **fixed at Oldest first**, Count **≤ 6**, this design's own ceiling — with `title` ← post title, `body` ← custom excerpt, `image` ← feature image, **each bound step linking to its post**, and Add, Remove and drag hidden: **P0·3's read-only card**. `label`, `duration` and `icon` have no Ghost source and stay empty.  0 and 1 → does not render. 2 to 6 → drawn. **7 or 8 → 14 Index.** In bound mode every box carries the plate and every right-hand figure is kept.

**Empty** · **no icon → the numeral plate in the 44 px box, and the right-hand figure is kept** — unlike 4 Cards, because the right-hand column of figures is the design and dropping one would break the column. No body → a shorter row. **At Icons None the rows close up by 64 and the design is 14 Index inside a panel**, which the panel says in those words.
...
- **Counts.** **Three to six.** **Two → 7 Alternating Media**, because two tabs are a toggle. **Seven or eight → 3 Rows**, because a seventh tab makes the column taller than the panel it controls.
- **Zero.** The section does not render.
...
**Data** · **Source: Authored · From posts.** Authored is the default and every frame's state. At From posts it is **P0·5 configured for reading paths** — Filter **By tag**, Order **fixed at Oldest first**, Count **≤ 6**, this design's own ceiling — with `title` ← post title, `body` ← custom excerpt, `image` ← feature image, **each bound step linking to its post**, and Add, Remove and drag hidden: **P0·3's read-only card**. `label`, `duration` and `icon` have no Ghost source and stay empty. **In bound mode the tab stays the tab and the panel's title carries the post link** — one tab stop inside the panel rather than a link on a control. 0 and 1 → does not render. **2 → 7 Alternating Media.** 3 to 6 → drawn. **7 or 8 → 3 Rows.**
...
1 Three Up with a picture above every step. **The design 8 Rail hands off to when its cards fit the content width.**
...
- **Add.** Lands last. **A fifth add changes the design** and the panel names 8 Rail before the edit.
- **Remove.** Never disabled; re-numbers.
- **Reorder.** Meaningful; pictures move with their steps.
- **Counts.** **Two to four.** **Five or more → 8 Rail**, and **8 Rail hands back here when its cards fit** — the one two-way hand-off in A13.
- **Zero.** The section does not render.
...
- **Remove.** Never disabled; the band narrows and the sentence and link stay pinned to their ends. **Below three → 1 Three Up**: two pills and a chevron is a toggle.
- **Reorder.** Meaningful; re-numbers.
- **Counts.** **Three to five, and the ceiling is measured rather than chosen.** Four pills at Filled with a lead-in and a link fill about 1,240 of the 1,296; five fit only with short titles. **Six or more → 2 Track**, which is this design with room for bodies.
- **Zero.** The section does not render.
...
**Editing** · **the lead-in sentence — `title` ≤ 104, drawn at 15 px and never as a heading — and the link label edit inline with P0·1**; `linkUrl` opens the **Link Picker**; the section link takes an optional **P0·2** icon. **Per step the pill's title edits inline in the pill** and never wraps, so **the field states the measured width rather than clamping**. **`icon` is a P0·2 icon slot** with the size (Small 20 · Medium 24) and colour role (Muted · Text) popover, drawn as a glyph with no box; **an empty slot draws the numeral in the glyph's 16 px place**, so that pill shows its figure twice and both are kept. **`eyebrow`, `sub` and `note` are held by this design and are not editable here** — there is nowhere on the band to draw them. The numeral is never editable.
...
Four to eight steps as hairline rows with the numeral, the label, the title and the body in aligned columns. **The design four of the other fourteen name when a site adds a seventh step** — A13's equivalent of A12's 1 Grid.

**Descriptor.** The only design that draws every field of every step in aligned columns down the whole section — numeral, label, title and body in four hard columns on hairline rows — and the design four of the other fourteen name when a site reaches seven steps.
...
- **Remove.** Never disabled; the rows close up and everything after re-numbers. **Under four → 1 Three Up.**
- **Reorder.** Meaningful and **scanned immediately, because the figures are a visible column eight deep.**
- **Counts.** **Four to eight, and the design is built for the top of that range.** Below four, four aligned columns have nothing to align.
- **Zero.** The section does not render.
...
- **Remove.** Never disabled; re-numbers both columns. **Under four → 3 Rows.**
- **Reorder.** Meaningful; re-numbers both, **and changes which step the reader lands on from a given list row.**
- **Counts.** **Four to eight.** Below four a sticky list of three is taller than the thing it indexes. **This design absorbs the eight-step ceiling.**
- **Zero.** The section does not render.
...
**Responsive** · **at 834 the list becomes a sticky horizontal bar under the head** — hairlines above and below, scrolled by touch, the current step marked with a 2 px bottom border and **scrolled into view as it changes, the one place in A13 a behaviour moves something the reader did not touch.** **At ≤ 767 the list is dropped entirely and the design is 3 Rows**, with `scroll-spy` not declared at that width — **the only design in A13 that drops its defining element rather than turning it** — because a sticky bar on a phone costs 56 px of a 640 px viewport to say something the scroll already says.

**Data** · **Source: Authored · From posts.** Authored is the default and every frame's state. At From posts it is **P0·5 configured for reading paths** — Filter **By tag**, Order **fixed at Oldest first**, Count **≤ 8**, this design's own ceiling — with `title` ← post title, `body` ← custom excerpt, `image` ← feature image, **each bound step linking to its post**, and Add, Remove and drag hidden: **P0·3's read-only card**. `label`, `duration` and `icon` have no Ghost source and stay empty. **In bound mode the list rows stay anchors to the steps and the step title carries the post link** — the anchors are navigation and are never replaced by outbound links. 0 and 1 → does not render. **Under four → 3 Rows.** 4 to 8 → drawn. Over eight the repeater stops.
...
8. **The category has one two-way hand-off**, which is new: 8 Rail → 12 Media Top when the cards fit, and 12 Media Top → 8 Rail at five steps. Every other hand-off in the library so far has been one-directional.
...
| # | Design | Media | Reads beyond the floor | Hands off |
|---|---|---|---|---|
| 1 | Three Up | none | — | 14 Index at 7+ |
| 2 | Track | none | — | 1 Three Up at 6+, and at 2 |
| 3 | Rows | none | `body` ≤ 400, `label` | absorbs eight |
| 4 | Cards | icon | `icon` | 14 Index at 7+ |
| 5 | Split Head | none | section `body` | 1 Three Up with no title, 3 Rows at 7+ |
| 6 | Contrast Band | none | — | 1 Three Up in print and forced colours |
| 7 | Alternating Media | image | `image`, `imageAlt` | 3 Rows at 5+ |
| 8 | Rail | image | `image`, `imageAlt` | 12 Media Top when the cards fit |
...
| 14 | Index | none | `label` | receives from four |
| 15 | Sticky Rail | none | section `body`, `body` ≤ 400, `label` | 3 Rows under four and at ≤ 767 |

**Eight designs draw no media at all, three draw an icon, four draw a picture. Four hand off to 14 Index at seven steps and four more to 3 Rows; four absorb the eight-step ceiling. Three have a behaviour and twelve have none.**
...
- **Count classes are read at the count the design is drawn at** — A12's stated departure, carried. Six steps is `many`, three or four is `few`, and **8 Rail is `variable` because its precondition is a width rather than a total.** *Flagged: the departure is mine.*
...
|---|---|---|---|
| 8 Rail | `carousel` | yes — parked, position never persisted | registry: native scroll-snap strip, arrows and dots hidden. **Here:** exactly that — the snap is drawn, so the section loses only two arrows and two fades; **it does not become 12 Media Top** |
