...
const NOJS = `${b('Modules:')} ${code('accordion')} for the toggle card, ${code('core')} for everything else — ${b('the same two in all six treatments')}, because a treatment is a stylesheet ⚑. ${b('Edit-safe:')} yes; the resting state is the only state and nothing animates, posts or pins while the post is edited. ${b('No-JS, quoted:')} ${code('accordion')} — “Native ${code('&lt;details&gt;')} — fully functional, keyboard-operable, opens and closes with no JS at all. A9’s ${code('default state')} control resolves to the server-rendered ${code('open')} attribute.” ${code('core')} — “Never runs; the ${code('.js-enabled')} class is never set, so all JS-conditional CSS stays in its no-JS branch.” ${b('The one gap is Ghost’s own gallery row script')} ⚑ — it computes each row’s ratios, the registry has no module for it, and with JS off the images render at equal widths. That is a finding for the architect, not a degradation A33 may quote.`;
...
const d1 = {
  ...
  statesNote:`${b('One hover per card and never two')}. The bookmark’s whole card is one link, so ${b('its hover is the surface stepping to the hover token and nothing moving')} ⚑ — no lift, no scale, no shadow appearing. The button card derives its hover from the accent at 93 % brightness, A32’s rule carried. ${b('Focus is A6’s 2 px accent ring at a 4 px offset')} in every card that is focusable, and the toggle is a native ${code('&lt;details&gt;')}.`,
  ...
  spec:[[
    ...
    specRow(10, 'Accessibility', `${code('&lt;figure&gt;')}/${code('&lt;figcaption&gt;')} kept paired on image, gallery and embed. Bookmark: one ${code('&lt;a&gt;')}, one focus stop, thumbnail ${code('aria-hidden')}, title as the accessible name. Toggle: ${code('&lt;details&gt;')}/${code('&lt;summary&gt;')}, ${b('the heading inside is a')} ${code('div')} ${b('and never an h-level')} ⚑. Button card: ${code('&lt;a&gt;')} at 48 px, 44 px minimum target. Product: the rating is text as well as stars. File: the link names the file and its size. Header: ${code('h2')}, never promoted to ${code('h1')}. ${b('The hairline carries no role')}; the callout’s emoji carries ${code('aria-hidden')}. Focus order is document order in every treatment ⚑.`),
