...
Frames: `A2-1 Rule` · `A2-2 Split` · `A2-3 Badge` · `A2-4 Two-Line` · `A2-5 Capture` · `A2-6 Countdown` · `A2-7 Dateline` · `A2-8 Ticker` · `A2-9 Rotator` · `A2-10 Pill` · `A2-11 Toast` · `A2-12 Takeover` · `A2-13 Notice` · `A2-14 Edge` · `A2-15 Triple` (all `.dc.html`).
...
**The item list.** One repeater runs through the category: **`messages[]`, the 2–6 list A2·8 Ticker, A2·9 Rotator and A2·15 Triple all read** — the same list, so switching between the three keeps everything. The union item shape is `message` (req, ≤ 120) · `messageShort` (opt, ≤ 48) · `label` (opt, ≤ 14, added to the item shape by Triple) · `linkLabel` (opt, ≤ 24) + `linkUrl`. Each design draws what it draws and keeps the rest: Ticker draws neither `messageShort` nor `label`, Rotator does not draw `label`. **The other twelve designs draw one notice from `message` and carry no repeater at all**, and their Items field says so rather than leaving it to be inferred.
...
A small card in a bottom corner, arriving after the page has settled. The only design that appears rather than being there, and one of two — with A2·13 Notice — that never touch the header.
...
**Placement against other fixed things.** The card never covers a fixed element the site already owns — it stacks 12 px above the Notice bar or a bottom player, and the Notice sits first.
...
**Flagged as mine.** The 60-character title cap, the 2-second default, the omission of exit-intent, the dark shadow value, stacking 12 px above the Notice, and the full-width mobile primary that contradicts A2·2 for a stated reason.
...
## 13 · Notice — renamed from Consent this pass

A bottom bar carrying an informational notice: one sentence, a policy link, and a dismiss. Resolved per the owner's ruling: the theme ships no third-party scripts and cannot manage cookie consent for scripts the owner injects, so the Accept/Decline pair is removed and the design is renamed. **This bar records nothing and gates nothing** — the spec's honesty line survives the rename. A real consent module (choice storage + a DOM event + gating `<script type="text/plain" data-consent>` code-injection blocks) is recorded as the upgrade path — **ARCHITECT: registry addition (`consent`)**, on the frame and here — not drawn.

**Descriptor.** The only bottom-anchored bar in the top-stack's vocabulary — one sentence with an always-underlined policy link and a dismiss, recording nothing and gating nothing.

**Structural descriptor.** `bar · none · surface · one · none · sentence with policy link`

**Archetype.** bar

**Behaviour module.** `dismiss` — a fit at last. The old finding (“one question with two answers whose stored values differ”) closed with the buttons' removal: what remains is one close with a per-browser memory, which is exactly the module. **JS off:** “The bar renders and stays; the close button is hidden rather than rendered inert.” The sentence and the policy link render and work; what a reader without JavaScript loses is the ability to put the bar away — an honest degradation the unanswerable consent question could never have.

**Items.** None. One sentence, one link, one close. `messages[]` is kept.

**Content fields.** `message` (rich text, req, ≤ 200 — the category's one longer cap, because a notice has to say what it does; carries the policy link inline, edited with the P0·1 toolbar and its Ghost-aware link popover, new-tab and rel options included) · `linkLabel` + `linkUrl` (opt pair — rendered after the sentence when Policy link in text is Off). `ctaLabel`, `secondaryLabel` and `askAgainAfter` are retired from this design; `dismissible` and `audience` return to the standard Visibility group, and `dismissMemory` replaces `askAgainAfter`, defaulting here to **Never comes back**.
...
The old Actions row went with the buttons; the old Padding row was the universal Vertical spacing under another name and is removed in its favour (§0); Ground merged into the universal Background role (Surface · Contrast). Top divider locks None — a bottom-anchored bar's top hairline and inverted shadow are the design's own edge, not a seam.

**Data.** Member state, for the restored `audience`. This design and A2·11 Toast are the two that may coexist with one top bar; when both are present the Notice sits first, 12 px below the Toast.

**Member ask.** **None.** The only link in this design is the policy link, and a policy is not an ask — so the two conditional notes and the no-JavaScript form notice have nothing to attach to, and **no Ghost membership setting affects whether this bar renders**. Stated rather than left blank, because this is the one design in the category where the answer is nothing. Its own no-JavaScript degradation is unchanged: the bar renders and stays, and the close is hidden.

**Responsive.** 1440–768 one row: sentence left, close right, 18 px padding, top hairline and an inverted md shadow. Boxed centre puts the sentence in a 720 px card 20 px above the bottom edge; Boxed left pins the same card to the left margin; in both the close moves to the card's top-right. ≤ 767 the sentence wraps to two lines and the close is a full 44 px top-right target; bar ≈ 92 px including bottom padding for the safe-area inset. Nothing above the bar is ever offset or reflowed.

**Empty state.** Policy link off → the sentence carries no link and the policy is reachable from the footer, which the sidebar notes. No message → no bar.

**Accessibility.** `<aside aria-label="Site notice">` at the end of the body, present at load, so `aria-live` is off. Not a dialog: no focus trap, no focus move, no blocking. The policy link is underlined at rest — the one always-underlined link in A2, because a legal reference must not depend on hover. Close is `<button aria-label="Dismiss notice">` (a translation-catalog string), last in focus order; Escape does nothing — only Toast owns Escape; focus returns to where it was on dismissal.

**Flagged as mine.** The “Site notice” label, Never-comes-back as the default memory, and withdrawing this design's `#453E35` flag — with the Decline outline gone the invented dark border step is no longer needed here; A2·9's flag stands.
...
**Member ask (Part A·A9).** The link inside the **open** bar is the affordance, and pointed at sign-up it is a member ask; the chip is a disclosure and never an ask. It does not render when the connected site cannot honour it — signup closed, members disabled, or no payment provider for a paid ask — and the open bar is then the sentence alone, with the chip, the 4 px rule and the `<details>` disclosure untouched, because none of them is the ask. It opens Ghost's Portal, so **with JavaScript off nothing happens** — and **this is the one place this design's script-off independence stops**: the bar still opens without script, the ask inside it still does not work. **It renders when** members are on and signup is open; a paid ask needs a payment provider connected and a live paid tier as well; a sign-in link needs members on only.
...
| 13 | Notice | bar · none · surface · one · none · sentence with policy link |
...
**Containment is `none` thirteen times, and that is the honest answer.** A2's sections are bands, and a band is not a container — it is the section's own ground. Only **10 Pill** and **11 Toast** are contained, because in both the section *is* a detached object with the page ground visible around it. **Pill is `card`, not `pill`:** the design's own rule refuses the capsule — “the corners are the pack's radius token, like everything else in the library” — so the name is the design's, not the tuple's. 13 Notice at Layout: Boxed centre is the one place a box could have been claimed, and that is a control value on a design whose default is full bleed, not a second design.

**Ground does the most separating in A2** — four values across fifteen designs, and it is the drawn default in every case, never the control's full range. `contrast` on 1, 4, 6, 8; `surface` on 2, 9, 12, 13, 15; `page` on 3, 7, 10, 11; `accent` on 14 alone, because the rule *is* the design. 1 Rule offers Surface and Accent as control values and is still `contrast`. **`image` and `transparent` are unused:** 12 Takeover's Image: Background is a control value on a surface band, and the category rule forbids a transparent or scrimmed bar over a hero outright — “The bar is never given a transparent or scrim treatment.”

**Item count: the repeating unit in A2 is the notice.** Twelve designs show exactly `one`. 8 Ticker and 9 Rotator take the shared 2–6 list and are `variable`, since the author decides how many. 15 Triple is `few` because its Slots control fixes two or three whatever the list holds — the items beyond the slot count are kept and counted in the sidebar, not shown. `none` and `many` are unused: there is no A2 design without a notice, and the 6-message cap keeps every list under five shown at once.

**Media placement is `none` fourteen times.** 12 Takeover is the category's only imagery and is `left` — its drawn default panel. Background is a control value on that same design, not a second one, and `imageAlt` is the only image field on the category list.
...
Seven of the registry's 31, and nothing coined. `dismiss` appears on all fifteen designs — 13 Notice joined it this pass when the owner's ruling replaced the Accept/Decline pair with a dismiss.
