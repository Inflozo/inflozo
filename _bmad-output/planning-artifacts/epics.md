...
2026-09-22): any section of page 2's design can be picked and changed, pages 3, 4, 5… carry the change, and no page 3
or later is offered
...
**And** **the nudge timer is a no-response timer, not a decision timer: it RESTARTS from the holder's last
interaction with the popover, focus included — it does not stop** (UX-DR13 as F-079 rules it)
...
for at least an hour, so a refusal is never retried (MEASUREMENTS §51, both majors; the hour timed in §52), and Ghost(Pro) sits behind an
