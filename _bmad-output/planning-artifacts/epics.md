...
- **UX-DR19 · Component behaviours** — greyed control · absent control · item list · design picker · section on canvas · inline toolbar (a mark a field does not permit is **absent, not greyed**, with a fixed button order) · site-wide singleton with its page count and its affects-every-template confirm · Pro badge (**no upgrade sheet on click, ever**) · persistence indicator (four labels, one dot, never a spinner) · feedback banner · wizard step rail (4 steps normally, **6 on the first deploy to a site**) · typed confirm, only where the action is irreversible and account-wide.
