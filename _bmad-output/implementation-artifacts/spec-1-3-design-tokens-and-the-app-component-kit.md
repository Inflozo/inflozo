...
- Icons are inline `<svg>` paths in `currentColor` at 1.5px stroke (R-26). *Amended by the owner on 2026-09-05 (R-92, question 2 below): the app's icons are the frames' own drawings, read verbatim under R-74; Tabler (D1) and its MIT notice are the sections library's — the Icon Picker and the sites customers build — and do not apply to Inflozo's chrome.*
...
- [x] `apps/web/components/kit/icons.tsx` -- the icons the Kit draws, its own paths read verbatim, inline `<svg>` in `currentColor`, stroke 1.5, `aria-hidden` unless labelled; the header states their provenance and R-92's scope (no Tabler notice — none of its paths are here); membership derived from the Kit, never fixed here
...
**Ruled (owner, 2026-09-05): option 1 — keep the frames' drawings and correct the notice, and a scope clarification recorded as R-92: Tabler is the sections' icon set (the Icon Picker and the glyphs in the sites customers build), not Inflozo's own chrome.** The icons file now states its paths are the frames' own and carries no Tabler notice; the notice returns with the first Tabler path, in the library. Propagated the same day to this spec's constraint above, `epic-1-context.md`, `DESIGN.md` § Icons and the rulings register.
