...
**Why copy-and-guard rather than move.** The architecture's three SQL files are the design authority and are cited by `record` documents by path, which standing rule 5 forbids editing — so they cannot move. The spine names `supabase/migrations/` and `supabase/tests/rls.sql` as real files that must exist. The reconciliation is the repo's established pattern (`ux-designs/prototype/build.py` asserts every lifted region byte-identical to its source): the `supabase/` files are copies, and `cmp -s` in the gate is the standing-rule-7 audit that keeps them from drifting. A future schema change edits the authority and re-copies; the gate catches a stale copy. **Superseded 2026-09-05 by the owner's ruling on question 2 and by DW-8:** the migration is frozen and later changes are new files, so the gate no longer byte-compares the migration — it compares the database the migrations build against the one `SCHEMA.sql` builds. The `rls.sql` and `prelude.sql` byte-guards are unchanged.
