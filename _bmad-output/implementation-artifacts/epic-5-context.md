...
- **CSP.** The app host uses a nonce with `'strict-dynamic'` and `frame-ancestors 'self'`; `connect-src` has been a static `'self' https:` since Story 3.2 (`apps/web/csp.ts`), though the PRD and spine still say per-session.
