 ...
 *
 * A ROW THAT IS NOT THE CALLER'S IS A 404, never a 403: a 403 would confirm the id exists.
 */
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: signed, error: signError } = await supabaseAdmin()
    ...
    .createSignedUrl(snapshotObjectKey(snapshot.storage_path), SIGNED_URL_TTL_S)
