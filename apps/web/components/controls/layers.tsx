...
import { Copy, Eye, EyeOff, Moon, Pencil, Star } from '@/components/kit/icons'
import { TextInput } from '@/components/kit/input'
import { AutoGeneratedRow, FeedlessNote } from '@/components/editor/auto-generated'
import { MainFeedChip } from '@/components/kit/badge'
...
/* B7 · LAYERS (`B Missing Surfaces.dc.html`:1538-1580), AS R-126 AMENDS IT — the panel body, and Story 5.4's whole
   ...
   R-133 (owner, 2026-09-18) EXTENDS R-126 RATHER THAN REVERSING IT: the `⋯` gains a fifth item, "Clear dark
   overrides", by the owner's own word. The `⋯` is still the row's only control and Hide/Show still leads its menu.
   The item is ABSENT where the section carries no usable override — UX-DR3, and exactly the shape Duplicate already
   uses on a site-wide row below — because a menu item explains nothing about itself, while the Controls panel's row
   beside "Reset this design" is always present and says there is nothing to clear. Both open the SAME confirm, which
   is `editor.tsx`'s for Delete's and Hide's reason.

   STORY 5.19 ADDS TWO OF D5c's PIECES AND KEEPS R-126: the main feed's row carries D5c's MAIN FEED chip before its
   `⋯`, and a visible secondary feed of a paginated page offers "Make this the main feed" SECOND in its menu, after
   Hide/Show — absent everywhere else (UX-DR3). The Layers menu keeps its built words, Duplicate and Delete, not D5c's
   "Duplicate section / Remove section" (R-170, a routine call stated in the spec). And a Tag or Author page with no
   visible feed says so at the panel's head, in D5a's marker's shape.
...
export type LayerRow = {
  ...
  at: number
  /** Story 5.6, R-133 — does this section carry a dark override an emitter could use? The engine's own answer
   *  (`darkOverridesInForce`), derived by the caller. False means the menu item is ABSENT, not greyed. */
  darkOverride: boolean
  /** Story 5.19 — this is its page's MAIN FEED: D5c's chip sits on the row, before the `⋯` */
  mainFeed: boolean
  /** Story 5.19 — **Make this the main feed** is offered: a visible feed of a paginated page that is not already it.
   *  False means ABSENT (UX-DR3), on the main feed, a hidden row, a section that is no feed and any other page. */
  canLead: boolean
}
...
export type LayersProps = {
  ...
  templates: number
  /** Story 5.5 — this canvas is untouched, so D5a's marker leads the panel — the marker's ONE place since R-130 */
  autoGenerated: boolean
  /** Story 5.16 — the marker's own sentence, where it is not D5a's: page 2's "Copy of page 1…" while it follows */
  markerWords?: string
  ...
  /** Story 5.6, R-133 — opens the editor's ONE "Clear dark overrides" confirm for this row */
  onClearDark: (row: LayerRow) => void
  /** Story 5.19 — D5c's **Make this the main feed**: one edit, announced by the editor */
  onMakeMainFeed: (row: LayerRow) => void
  /** Story 5.19 — FR-H2's archive case: the sentence for a Tag or Author page with no visible feed, or null */
  ...
  return (
    ...
    >
      {/* D5a (:54-57): the marker is about the CANVAS, not a group, so it sits ABOVE both — R-126 drew the two groups
          and their hairline and this takes nothing from either heading. It goes with the first edit (AD-22). */}
      {autoGenerated ? <AutoGeneratedRow words={markerWords} /> : null}
      {/* Story 5.19 — FR-H2's archive case, beside D5a's marker and in its shape, while it is true */}
      {feedless === null ? null : <FeedlessNote words={feedless} />}
